...
## Pair 6 — Street FY+1 is the default Y1 start

Next-year Street revenue is the **default Y1 start** (`street_baseline`). Do not invent a destock or duration FY+1 and treat Street as a footnote unless a resolved `independence_gate` applies. Street **price targets** are still not a DCF input.
...
**Why bad:** |delta| 29% (and even 7%) is a machine FAIL on `street_baseline`. `keep_independent_vs_street` is illegal. Haircut belongs in **bear**.
  ...
  "street_hooks": [
    {
      ...
      "reason": "Street FY+1 revenue is the default base Y1 starting point."
    }
  ]
}
```

**Why good:** Y1 *is* Street. Needle present. Destock/haircuts live in bear. Historical independent-then-calibrate (2.7–2.17): `harness/law_history.md`.
