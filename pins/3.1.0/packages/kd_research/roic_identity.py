...
def session_is_roic_runtime(session: Path) -> bool:
    ...
    return session_since(session, ROIC_SINCE)


def session_is_midcycle_runtime(session: Path) -> bool:
    return session_since(session, MIDCYCLE_SINCE)
...
def check_roic_if_vm(session: Path) -> list[tuple[str, str, str]]:
        ...
        if len(reason.strip()) < REASON_MIN:
            out.append(
                (
                    ...
                    "applies:false requires not_applicable_reason (≥40 chars; native analog for banks/insurance/growth; REIT keeps ROIC on ≥ 3.1.0)",
                )
            )
            return out
        analog = ident.get("native_analog")
