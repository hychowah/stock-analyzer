...
| F30 | Destock analog in **base** while Street FY+1 is usable (no this-print analog) | 2.12 destock-in-base teaching left in live prompts after 2.18 inverted Y1 | W1 — **2.18–2.27** `check_destock_*` FAILs destock-in-base while Street usable. On ≥2.28 see F34. |
| F34 | Destock-in-base without `destock_this_print`, or `independent_y1` without a resolved gate, or `street_baseline` with \|delta\|>5% | 2.18 5% statute + destock-in-bear overcorrected independent appraisal; 2.28 consistency layer | W1 — 2.28.0 gated Y1: Street default; destock-in-base only when analog_class + current_print_is_destock; `ttc_midcycle` is not a Y1 license |
...
| F35 | Mechanical CAPM WACC with coupon Kd and trough market weights prints a utility-like CoC and licenses `franchise_mos` | `wacc_vs_buildup: matches_buildup` certified the formula, not the ingredients; Agent 5 session script hardcoded coupon as Kd; no Kd-vs-Rf or distressed-weight gate | W1 — 2.40.0 `wacc_buildup` identity (`packages/kd_research/wacc_buildup.py`): coupon illegal as Kd; pretax Kd ≥ Rf unless `negative_rate_market`; trough `current_market` weights illegal when `wd>0.35` and implied gap >200 bp; target wd may not copy trough wd |
