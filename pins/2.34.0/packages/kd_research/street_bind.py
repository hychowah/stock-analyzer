...
from packages.kd_research.valuation_hygiene import (
    check_conservatism_dials,
    check_same_period_box_floor,
    check_sotp_gap,
...
DELTA_THRESHOLD = CALIBRATION_BAND
DELTA_EPS = 0.005
...
def session_is_street_y1_runtime(session: Path) -> bool:
    ...
    return session_since(session, STREET_Y1_SINCE)
...
def session_is_gated_y1_runtime(session: Path) -> bool:
    ...
    return session_since(session, STREET_GATED_Y1_SINCE)
...
def check_street_bind(session: Path) -> list[tuple[str, str, str]]:
    ...
    if unavailable or (enforce and not has_street and _fetch_log_street_failed(session)):
        ...
        out.extend(rows)
        out.extend(check_conservatism_dials(vm, require=require_dials))
        out.extend(check_sotp_gap(vm, require_if_both=require_dials))
        return out
    ...
    if y1_runtime and street_col is not None and base is not None and abs(street_col) > 1e-12:
        out.extend(check_y1_stacking_pair(vm, abs((base - street_col) / street_col)))
    out.extend(check_sotp_gap(vm, require_if_both=require_dials))
    if y1_runtime:
