...
def primary_artifact_exists(session: Path, agent_id: str) -> bool | None:
    ...
    if not found:
        return False
    if agent_id == "5":
        from packages.kd_research.decision import session_is_wave2_runtime  # noqa: WPS433
...
def check_llm_model_identity(
    ...
    *,
    ...
    """
    from packages.kd_research.provenance import load_manifest_models  # noqa: WPS433
...
def entry_checks(
    ...
    *,
    ...
    from packages.kd_research.phase_graph import check_phase_graph_entry  # noqa: WPS433
    ...
    if phase_id == "2_parallel":
        ...
        if session_enforces_1d(session):
            ...
            from packages.kd_research.operating_path import check_operating_path_hooks
        ...
        if vm.is_file():
            from packages.kd_research.roic_identity import check_roic_identity  # noqa: WPS433
...
def check_1c_year_dive_complete(session: Path) -> list[tuple[str, str, str]]:
    ...
    from packages.kd_research.excerpt_check import (  # noqa: WPS433
...
def complete_checks(session: Path, phase_id: str) -> list[tuple[str, str, str]]:
    ...
    if phase_id == "1_parallel":
        ...
        from packages.kd_research.street_bind import check_street_fetch  # noqa: WPS433

        out.extend(check_street_fetch(session))
        from packages.kd_research.library import check_library_gates  # noqa: WPS433

        out.extend(check_library_gates(session, phase="1_parallel_complete"))
        return prefix + out
    if phase_id == "1c":
        from packages.kd_research.library import check_transcript_freshness  # noqa: WPS433

        return prefix + check_1c_year_dive_complete(session) + check_transcript_freshness(session)
    if phase_id == "1d":
        from packages.kd_research.operating_path import check_1d_complete  # noqa: WPS433

        from packages.kd_research.consumption import check_1d_ind_background  # noqa: WPS433

        return prefix + check_1d_complete(session) + check_1d_ind_background(session)
    if phase_id == "2_parallel":
        out = []
        for rel in (
            "registry/technical.json",
        ...
        from packages.kd_research.consumption import (  # noqa: WPS433
            check_2d_street_cite,
            check_fdd_material_hooks,
        )

        out.extend(check_fdd_material_hooks(session))
        out.extend(check_2d_street_cite(session))
        from packages.kd_research.operating_path import check_operating_path_hooks  # noqa: WPS433
        from packages.kd_research.street_bind import check_street_bind  # noqa: WPS433
        ...
        from packages.kd_research.decision import (  # noqa: WPS433
        ...
        from packages.kd_research.epistemology import (  # noqa: WPS433
        ...
        return prefix + out
    if phase_id == "2_5":
        from packages.kd_research.consumption import check_stress_legal_dollar  # noqa: WPS433

        return (
            prefix
            + check_stress_coverage(session)
            + check_latest_quarter_risk_mapping(session)
            + check_stress_legal_dollar(session)
        )
    if phase_id == "4_parallel":
        ...
        if session_is_wave2_runtime(session):
            out.extend(check_decision_packet(session))
            from packages.kd_research.decision import check_readme_quotes_decision  # noqa: WPS433

            out.extend(check_readme_quotes_decision(session))
        out.extend(check_wave6_reopen(session))
        out.extend(check_stress_bind(session))
        return prefix + out
