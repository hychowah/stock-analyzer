...
from packages.kd_research.classification import CLASSIFICATION_REL, CLASSIFICATION_SINCE
from packages.kd_research.library import BIND_REL, LIBRARY_SINCE
from packages.kd_research.narrative_lock import NARRATIVE_3P_REL, NARRATIVE_REL
...
PHASE_GRAPH: tuple[PhaseNode, ...] = (
    _n(
        ...
        stage="setup",
        purpose="KPI sector, engine classification, market context, research brief, library bind",
    ),
    _n(
        "0",
        ("orch",),
        ...
        entry_versioned=(VersionedPath(CLASSIFICATION_REL, True, CLASSIFICATION_SINCE),),
    ...
    _n(
        ...
        complete_paths=(NARRATIVE_REL, NARRATIVE_3P_REL),
        primary_artifacts={
            "story": (NARRATIVE_REL,),
            "3p": (NARRATIVE_3P_REL,),
        },
        ...
        label="Story lock",
        stage="decide",
        purpose="Story + 3P critic lock before DCF compute",
        handoff_globs=(
            ("story", ("story*.md", "1e_story*.md")),
            ("3p", ("3p*.md", "1e_3p*.md")),
        ),
        subagent_priors=(
            ("story", ("0", "1b", "1c", "1d")),
...
def graph_for_session(session: Path | None = None) -> dict[str, PhaseNode]:
    ...
    omit_1e = False
    if session is not None:
        from packages.kd_research.narrative_lock import session_enforces_narrative

        omit_1e = not session_enforces_narrative(session)
    if omit_1e and "1e" in nodes:
        nodes.pop("1e")
        two = nodes["2_parallel"]
        # Ready-set live graph: Agent 5 waited on 1e. Restore 1d only when
        # 1d still exists and per-subagent ready-sets were not already cleared
        # by the pre-3.0 overlay (that overlay shares 2_parallel after 1d).
        if "1d" in nodes and two.subagent_priors:
            priors = dict(two.subagent_priors)
            priors["5"] = ("1d",)
            entry = dict(two.subagent_entry)
            five = tuple(
                p
                for p in entry.get("5", ())
                if p not in {CLASSIFICATION_REL, NARRATIVE_REL}
            )
            if five:
                entry["5"] = five
            nodes["2_parallel"] = replace(
                two,
                subagent_priors=tuple(priors.items()),
                subagent_entry=tuple(entry.items()),
            )
