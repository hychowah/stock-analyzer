...
- `scripts/check_catalog.py` (B7, not yet written) verifies every index path exists and reverse: every session dir appears in index (after migration). That is not `packages.kd_research.check_catalog`, which is the preflight/merge when-tag table.
