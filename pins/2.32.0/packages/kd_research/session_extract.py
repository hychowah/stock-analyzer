...
def _extract_decision_action(session: Path) -> str | None:
    from packages.kd_research.decision import extract_decision_action  # noqa: WPS433

    return extract_decision_action(session)


def _extract_kill_triggers(session: Path) -> list[str]:
    from packages.kd_research.decision import extract_kill_triggers  # noqa: WPS433
...
def extract_session_bundle(session: Path) -> dict[str, Any]:
    ...
    from packages.kd_research.roic_identity import thin_roic_from_valuation  # noqa: WPS433
