...
**Why bad:** On 2.18–2.27 Street FY+1 is the required Y1 start; destock-in-base while Street is usable FAILs. On ≥ 2.28 destock-in-base is still FAIL unless `destock_this_print` (this JSON does not show that gate). (On 2.12–2.17 this pairing was the GOOD example: destock default was base until cash/channel proved demand. That wording belongs in this caption, not in a live `reason` field.)
...
**Why good:** Street is the default Y1 (`used_as:fy1_baseline`). Destock analog is `applies_in: bear_only` when this print is not destock. Material OM recommendation is `rejected`. Illustrative numbers are style only. On ≥ 2.28 destock-in-base is legal only with `destock_this_print` (`analog_class` + `current_print_is_destock=true`), not as a free preference.
