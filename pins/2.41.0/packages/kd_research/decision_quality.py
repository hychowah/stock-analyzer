...
from packages.kd_research.roic_identity import _cheap_class

WAVE1_SINCE = (2, 9, 0)
ROC_JOIN_SINCE = (2, 32, 0)
...
def session_is_wave1_runtime(session: Path) -> bool:
    return session_since(session, WAVE1_SINCE)
...
def check_roc_screen_vs_cheap_claim(session: Path) -> list[tuple[str, str, str]]:
    ...
    if cheap == "franchise_mos" and bucket == "above_wacc":
        ...
        if isinstance(dec, dict) and dec.get("roc_screen_rebuttal"):
            rebuttal = dec.get("roc_screen_rebuttal")
        ...
        join_fail = parsed is not None and parsed >= ROC_JOIN_SINCE
        if not isinstance(rebuttal, str) or len(rebuttal.strip()) < 40:
            return [
                (
                    "FAIL" if join_fail else "WARN",
                    "roc_vs_cheap_claim",
                    ...
                    "(≥40 chars on decision.json at 5b, or on roic_identity if TSR was already on disk)",
                )
            ]
        return [
            (
...
def check_wave1_decision_quality_core(session: Path) -> list[tuple[str, str, str]]:
    return check_wave1_decision_quality(session, include_reports=False)


def check_wave1_decision_quality_full(session: Path) -> list[tuple[str, str, str]]:
    return check_wave1_decision_quality(session, include_reports=True)
