...
|-------|--------|-------------------------|------------------|
| **orch** | main | `sector_config.json`, `market_context.json`, `classification.json` (≥ 3.4.0), `research_brief.json` (new sessions) | KPI sector + engine card (playbook, life-cycle, job). **§5 identity; modules advisory** (detection lists do not classify). |
...
| **1d** | 1d_rev ∥ 1d_ind ∥ 1d_ol then 1d_merge | `raw/oppath_*.json` + `operating_path_brief.json` (after Phase 0 + 1b; not 1c) | Growth/industry/leverage facts + conflict map for the story lock |
| **1e** | story then 3p | `narrative_bind.json` (story/map/hooks) + `narrative_3p.json` PASS (after 0+1b+1c+1d; ≥ 3.4.0) | 3P PASS is the lock. Agent 5 cannot start without it. |
| **2_parallel** | 4, 5, 12 | `technical.json`, `valuation_model.json` (`shock_surface.dials` on ≥ 2.44.0), `tsr_validation.json` | FV/MOS for reports & stress; levels for technical report. Agent 4/12 do not wait on 1d/1e. Agent 5 waits on 1e. |
...
| `latest_quarter` or `filing_deep_dive` | 1e story / Agent 5 | 2d / 1c (year-readers + 2e) |
| `operating_path_brief` | 1e story | 1d workers + 1d_merge (after Phase 0 + 1b) |
| `classification.json` | Phase 0 / 1e / Agent 5 | Orchestrator after sector + market_context (≥ 3.4.0) |
| `narrative_3p.json` PASS | Agent 5 | 1e story then 3p critic (≥ 3.4.0). Identity is `classification.json`, not the bind. |
...
python3 scripts/preflight_phase.py --ticker T --date D --phase 1e
python3 scripts/preflight_phase.py --ticker T --date D --phase 1e --mode complete
...
| Engine classification | `harness/schemas/classification.schema.json` |
| Narrative lock / 3P critic | `harness/schemas/narrative_bind.schema.json`, `narrative_3p.schema.json` |
| Damodaran catalog identities | `packages.kd_research.damodaran_gates` (≥ 3.1.0: required playbook + TV method; price column is not FV; ≥ 3.2.0: narrative substance + map to model inputs + weighted truncation + closed pricing roles + unstamped-v3 FAIL; ≥ 3.3.0: buyer/life-cycle/option screens/market-neutral/per-share + a required truncation answer) |
