...
This module manages the comparison warehouse sqlite (catalog_sqlite_path:
local home in production, archive/catalog in tests) as a rebuildable projection.
"""
...
""",
    3: """
ALTER TABLE runs ADD COLUMN quote_symbol TEXT;
ALTER TABLE runs ADD COLUMN quote_listing TEXT;
ALTER TABLE runs ADD COLUMN quote_listing_source TEXT;
""",
}
...
def db_path(output_dir: Path | str | None = None) -> Path:
    ...
    return catalog_sqlite_path(output_dir, filename=DB_FILENAME, create=True)


def connect(output_dir: Path | str | None = None, *, readonly: bool = False) -> sqlite3.Connection:
    """Open compare DB. Writers use WAL + busy_timeout; readers use mode=ro URI."""
    if readonly:
        path = catalog_sqlite_path(output_dir, filename=DB_FILENAME, create=False)
        if not path.is_file():
...
def row_from_export_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "quote_listing": payload.get("quote_listing"),
        "quote_listing_source": payload.get("quote_listing_source"),
    }
