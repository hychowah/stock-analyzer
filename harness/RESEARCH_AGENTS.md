...
5. If `|delta_pct| > 0.20`, that is a **calibration note**, not a valuation skill miss: record `divergence_rationale` / `response` when you have them (machine WARN if omitted). `keep_independent_vs_street` still needs a transmission mechanism when you keep a large gap. Copying Street into the path remains FAIL. The harness never auto-sets `base = street`.
...
| New-runtime `street_estimates.json` + valuation: `street_bind` independent construction; `delta_pct` identity; `|delta|>20%` is a calibration WARN (not a skill-miss FAIL); `street_hooks` non-empty, not all-`noted_only`, no path-copy action. Absent on legacy → SKIPPED | `[machine]` `check_session --full` + `2_parallel` complete |
...
| Harness ≥ 2.12.0: destock conflict of any status cannot park destock in bear while duration stays in base (same legal set as 2.11: destock-in-base / DU=low / pass|too_hard); `1d_merge` default is destock-in-base; destock-inverse two-quarter raise is WARN. Legacy / <2.12.0 → SKIPPED | `[machine]` `check_session --full` |
