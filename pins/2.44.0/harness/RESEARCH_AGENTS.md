...
- Phase 1 agents 2a/2b/2c are genuinely parallel **with Phase 0**. 2d must run **after** 2a and 2b — it reads their outputs.
...
- **1d** runs after Phase 0 + 1b + 1c. Workers gather only (no 8-year forecast). `1d_merge` writes `operating_path_brief.json`. Do not average flatten vs destock.
- Phase 2 agents are parallel with each other but need Phase 1/1b/**1c**/**1d** complete for valuation. Agent 4 must not read any fundamental artifact (including `filing_deep_dive.json`, `market_context.json`, or `operating_path_brief.json`).
...
- Phase 2.5 needs `data/valuation_model.json`. Charts (phase 3) and reports (phase 4) start after 2.5. Phase 5 is last.
...
## 10c. Street FY+1 is the default Y1 start

**Street is the default Y1 start, not a statute that forbids disagreement.** Vendor FY+1 revenue consensus is the usual next-year level. Agent 5 does **not** invent an independent destock Y1 without a resolved evidence gate. Street **price targets** stay out of the DCF.

This section is **current law**. Agent 13 loads `harness/law_history.md` **only** when `harness_version < 2.28` (2.18 required-Street Y1; 2.7–2.17 independent-then-calibrate). Live Agent 5 / orchestrator must not load that file.
...
4. **`response`:**
   - `street_baseline` — default. `|delta_pct| > 0.05` is FAIL (you claimed Street and missed). `|delta| > 0.03` WARN. `street_hooks[]` must include `used_as:fy1_baseline` (or `used_as:revenue_path` / `street_mean`).
   - `independent_y1` — requires `independence_gate` that **resolves** against session artifacts. `|delta| > 0.05` WARN. `|delta| > 0.20` FAIL without `divergence_rationale` ≥40 chars. `used_as:fy1_baseline` is **not** required; Street still must not be all-`noted_only`.
...
5. **Legal `independence_gate` values** (machine: pointers resolve; `ttc_midcycle` is **not** a Y1 license):
...
7. **Destock analog:** destock-in-base is **legal** only when `destock_this_print`. Destock-in-base remains **FAIL** while Street is usable and this print is **not** destock. Destock analog in **bear** is always legal. Mid-cycle / TTC construction stays an ROIC identity control — it does **not** license Y1 to leave Street.
...
**Current machine gates only.** Version-banded Street/destock history is `harness/law_history.md` (Agent 13 when `harness_version < 2.28`). Python still SKIPPED-gates older stamps; live operators do not need those rows.
