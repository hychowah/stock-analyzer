...
def designed_phase_ids(session: Path | None = None) -> list[str]:
    """Phase ids this session must cover. Overlay lives on the graph."""
    from packages.kd_research.phase_graph import designed_phase_ids as _ids
...
def check_oppath_entry(session: Path) -> list[tuple[str, str, str]]:
    """2_parallel entry: brief exists + hooks when 1d is in force."""
    from packages.kd_research.check_core import check_path
