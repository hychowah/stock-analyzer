...
> **KPI / stress library — not the engine.** Playbook comes from `valuation_router.md` (a regulated utility is a `mature_operating` DCF unless the router says otherwise). The rate-base method below is a **modelling recipe**, not an engine choice; `fair_value.base` is a DCF, never a multiple or a rate-base "value".
