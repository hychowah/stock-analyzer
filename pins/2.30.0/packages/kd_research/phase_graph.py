...
from packages.kd_research.phase_status import PHASE_AGENTS, PHASE_IDS
...
def designed_phase_ids(session: Path | None = None) -> list[str]:
    ...
    from packages.kd_research.operating_path import designed_phase_ids as _designed
...
def session_is_phase0_parallel_runtime(session: Path | None) -> bool:
    ...
    if parsed is None:
        return False
    return parsed >= PHASE0_PARALLEL_SINCE
...
def prerequisites_for(phase_id: str, session: Path | None = None) -> list[str]:
    ...
    priors = order[:idx]
    if phase_id == "1_parallel" and session_is_phase0_parallel_runtime(session):
        return [p for p in priors if p != "0"]
    return priors
