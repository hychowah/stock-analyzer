...
def check_year_dives(session: Path) -> list[tuple[str, str, str]]:
    """Year-dive files + FDD rechecks. Does not emit the FDD exists row."""
...
def check_1c_year_dive_complete(session: Path) -> list[tuple[str, str, str]]:
    """1c complete: FDD exists+parse plus year-dives."""
    from packages.kd_research.check_core import check_path

    status, detail = check_path(session, "registry/filing_deep_dive.json")
    return [(status, "registry/filing_deep_dive.json", detail), *check_year_dives(session)]
