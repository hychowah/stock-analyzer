...
from packages.kd_research.outcomes import mechanical_scorecard
from packages.kd_research.roic_identity import check_roic_identity
from packages.kd_research.session_extract import (
    extract_key_risks,
...
class RocBindTests(unittest.TestCase):
    ...
    def test_232_above_wacc_without_rebuttal_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            s = Path(td)
            _stamp(s, "2.32.0")
            vm = _vm()
    ...
    def test_232_rebuttal_on_decision_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            s = Path(td)
            _stamp(s, "2.32.0")
            vm = _vm()
            ...
            _write(
                ...
                {
                    "ticker": "X",
                    "duration": {"action": "pass", "rationale": "Reopened after stress and TSR."},
                    "roc_screen_rebuttal": (
