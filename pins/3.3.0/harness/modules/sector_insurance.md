...
> **KPI / stress library — not the engine.** Playbook is `financial_service`. Equity DCF at ke. Never FCFF/WACC/EV as `fair_value.base`. Brokers included. P/B and EV/EBITDA are the price column.
...
Brokers are still `financial_service` (equity DCF at ke — never FCFF/WACC/EV as value). Multiples WORK as the **price** column because:
...
#### 2.5.1 Equity DCF at ke (broker-adjusted)
...
| | **Brokers:** Equity DCF at ke (broker-adjusted) | Brokers are `financial_service`: normal working capital, but value is discounted at **ke**, never FCFF/WACC/EV. |
