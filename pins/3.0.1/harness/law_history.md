...
**Not current law.** Live Agent 5 / orchestrator must **not** load this file. Agent 13 loads it when `S/meta/run_manifest.json` `harness_version` is `< 3.0.1` (3.0.0 independent Y1 default; Street default Y1 2.28–2.44; required-Street 2.18–2.27; independent-then-calibrate 2.7–2.17; 2.44 graph joins). Stress-bind policy is machine-banded in `packages.kd_research.decision` — do not paste the 25/15 cliff into a ≥ 2.44.0 session.
