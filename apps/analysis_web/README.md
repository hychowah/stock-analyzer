...
| `/api/portfolio/mtm-interval` | One still for a window. Pass `period=` (`1w`, `1m`, `ytd`, `statement`) or `start=` and `end=` (YYYY-MM-DD), not both. `pl` is value(end) − value(start) + IB fill cash after start through end. No frames, no bars. Heatmap window. User-initiated; not a poll |
| `/api/portfolio/mtm-path` | Daily reconstructed MTM frames. Pass `period=` (`1w`, `1m`, `ytd`, `statement`) or `start=` and `end=` (YYYY-MM-DD), not both. `pl` is value(t) − value(start) + IB fill cash after start; not the IB MTM file. Play only. User-initiated; not a poll |
