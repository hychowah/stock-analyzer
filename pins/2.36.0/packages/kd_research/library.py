...
from packages.kd_research.doc_text import convert_path
...
def session_enforces_library(session: Path) -> bool:
    if (session / BIND_REL).is_file():
        return True
    return session_since(session, LIBRARY_SINCE)
...
def check_library_entry(session: Path) -> list[tuple[str, str, str]]:
    if not session_enforces_library(session):
        return []
    return check_library_gates(session, phase="1_parallel_entry")


def check_library_complete(session: Path) -> list[tuple[str, str, str]]:
    return check_library_gates(session, phase="1_parallel_complete")


def check_library_session(session: Path) -> list[tuple[str, str, str]]:
    ...
    return check_library_gates(session)


def check_library_path_citations_full(session: Path) -> list[tuple[str, str, str]]:
    return check_library_path_citations(session, full=True)
...
def harvest_ticker(ticker: str, output_dir: Path | str | None = None) -> list[dict[str, Any]]:
    t = _norm_ticker(ticker)
    results: list[dict[str, Any]] = []
    for tick, _key, path in iter_research_sessions(output_dir):
        if tick != t:
            continue
