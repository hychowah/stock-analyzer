...
Scaffold **new** `S` → listing stamp + `verify_listing` → `bind_library.py` (before `sector_config`) → classify + `market_context` → `research_brief` → Phase 0 ∥ 1_parallel → **preflight** 1b/1c when files exist (not after 2c); 1d after 0+1b; Agent 4/12 skip 1d; Agent 5 after 1d+FDD; 2.5 / 4 / 5 as graph → audit → `finalize_session` (copies scaffold stamps).  
