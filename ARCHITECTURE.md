...
| `/portfolio/histories/{id}` | First paint is the paper book (lots and cash that day; Close/Value pending, not a blank price). Marks and the NAV path load as their own GETs — copy does not wait on Yahoo. Header cash and Δ are today (path end). The date picker is the paper account on D: closes on or before D, cash/stock/NAV, paper Reg-T loan/excess/buying power. Sell takes a visible quantity. Buy is a ticket at that day’s close. Actual is this copy, not a live IB walk. Does not call Live NAV. Works if the IB file is later missing. |
