...
def _bind_pin(job: dict[str, Any], pin: Any, archive_root: Path, workspace: Path) -> None:
    ...
    job["pin"] = {
        ...
        "label": pin.label,
        "root": str(pin.root),
        "copied_from_sha": pin.copied_from_sha,
        "harness_version": ident.get("harness_version"),
    }
    job["harness_root"] = str(pin.root)
    job["spawn_cwd"] = str(workspace)
    job["spawn_env"] = {
        "PYTHONPATH": str(pin.root),
...
def start_analyze(
    ...
    *,
    ...
    ws = Path(project_root or PROJECT_ROOT).resolve()
    try:
        pin = resolve(harness_version or "live", workspace=ws)
    ...
    if isinstance(backend, GrokSpawnBackend):
        ...
        if not runbook_has_ui_scheduled_heading(pin.root):
...
def resume_analyze(
    ...
    *,
    ...
    backend = spawn or default_analyze_spawn()
    ws = Path(project_root or job.get("spawn_cwd") or PROJECT_ROOT).resolve()
    pin_ver = "live"
    raw_pin = job.get("pin")
