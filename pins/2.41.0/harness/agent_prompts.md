...
Y1 LAW (current; do not load ROOT/harness/law_history.md): Street FY+1 is the **default** Y1 start (`response=street_baseline`; |delta|>5% FAIL). Independent Y1 is legal only with a named `independence_gate` whose session artifacts resolve: `destock_this_print` (analog_class inventory_channel|advertiser_budget AND current_print_is_destock=true), `definition_mismatch` (vendor_revenue_class ≠ filing_revenue_class), `native_kpi` (primary_sector banking|insurance|reit), or `street_unusable`. `ttc_midcycle` is not a Y1 license. Destock analog is bear-only unless destock_this_print — then destock may be base. Do not write a rationale that pretends a Street paste was independently built. Do not average destock and Street into one CAGR. `keep_independent_vs_street` is illegal. Do not paste module WACC/decay tables. Street **price targets** are not a DCF input.
...
2c. WACC BUILDUP (harness ≥ 2.40.0): write `wacc_buildup` (schema + `ROOT/harness/RESEARCH_AGENTS.md` §10e). Matching the CAPM formula (`wacc_vs_buildup: matches_buildup`) is required and is **not** a quality badge when Kd is a coupon or weights are the trough tape. `kd_source` is `current_yield` or `rf_plus_spread` only — coupon / stated coupon buckets are a cross-check, illegal as the WACC input. Pretax Kd ≥ session Rf (the Rf inside Ke) unless `kd_below_rf_gate=negative_rate_market` with `kd_below_rf_rationale` ≥40 chars and local Rf evidence. `rf_plus_spread` stores `spread_bp`; spread < 25 bp needs `kd_spread_hatch` (`gov_or_aaa` | `quoted_ytm_equals_rf` | `negative_rate_market`). Distressed tripwire: `wd > 0.35` **and** implied-WACC gap > 200 bp → `weight_policy` cannot be only `current_market`; `|wd − current_market_wd| < 2 pp` needs `structural_leverage_hatch` with a non-price basis. Skip Kd-vs-Rf when `wd < 0.05`. Do **not** paste a WACC/ERP/Kd number. Do **not** set model WACC to implied WACC (silent pad). Named Ke sleeves live inside Ke with use/reject; dual-class may be 0. `applies:false` with reason for banks/insurance/REITs (copy ROIC). Arithmetic 5 bp vs `assumptions.wacc` and `roic_identity.wacc`.
...
4d. When operating_path_brief.json exists: operating_path_hooks[] (used_as with old/new | rejected | noted_only). Material 1d recommendations may not be all noted_only. You may reject the brief; you may not skip consumption. Destock analog is **bear** unless `destock_this_print` (analog_class + current_print_is_destock=true) — then destock-in-base is legal via `response=independent_y1` and `independence_gate=destock_this_print`. Do not use DU=low / `pass` as a license to destock Y1 without that gate.
4e. STREET IS THE DEFAULT Y1 START. Default `response=street_baseline`: start base Y1 from Street FY+1; write street_bind (guide × street × base, delta_pct = (base−street)/street, y1_construction.rationale) and street_hooks[] with **used_as:fy1_baseline** (revenue_path / street_mean also legal). street_bind.street must equal street_estimates FY+1 when numeric. |delta|>5% FAIL on street_baseline. `independent_y1` requires a resolved independence_gate (see Y1 LAW). `keep_independent_vs_street` is illegal. Remaining-period company box **low** is a floor for that same period in base; a miss of an unprinted guide is bear. Profit: Street EBIT/OI if fetched on street_baseline; else EPS×shares with a bridge only if eps_basis is gaap|non_gaap; unknown-basis EPS is not EBIT. **Required** conservatism_dials[] with four keys (volume_vs_guide, gaap_om_vs_guide, sbc_in_fcff, wacc_vs_buildup); omit is FAIL. Do not silently stack ≥3 of those in base without stacking_justification. If SOTP and DCF both run, write multi_method_reconciliation (omit is FAIL); if |Δ|/primary > 40%, path_reopened + what_changed or gap_rationale.
...
OUTPUT CONTRACT — write S/data/valuation_model.json per ROOT/harness/schemas/valuation_model.schema.json including: model, fair_value (bear/base/bull/PW + dual MoS), assumptions, compute_script, sensitivity, filing_deep_dive_hooks, market_context_hooks (when MC present), operating_path_hooks (when 1d brief present), street_bind + street_hooks (when street_estimates present), conservatism_dials (required on harness ≥ 2.7.0), reverse_engineering; plus terminal_consistency / multi_method_reconciliation (required when SOTP and DCF both run) / research_brief_hooks / decision_usefulness / probability_method when triggered / **roic_identity (required on harness ≥ 2.8.0)** / **wacc_buildup (required on harness ≥ 2.40.0)**. Fair-value weights become risk_bridge scenario_probabilities (bare floats only; consistent).

SELF-CHECK before handoff: (1) abs(pct − 100×frac) < 0.05; (2) ≥3 rationales rehydrate values; (3) terminal_consistency if terminal method; (4) PFP names dials or explicit FALSE with dial evidence; (5) ERP/method competitor named if CoE used; (6) street_baseline uses used_as:fy1_baseline; independent_y1 has a resolved independence_gate; destock-in-base only if destock_this_print; (7) |delta_pct| > 0.05 is FAIL on street_baseline; keep_independent_vs_street is illegal; (8) street_bind.street matches the Street file; (9) four conservatism_dials keys present; (10) roic_identity present on ≥2.8.0 (applies:false with reason, or dual column + legal exit + cheap_claim); (11) mid_cycle_construction on ≥2.13.0 when applies:true (window licenses above_wacc/franchise_mos); (12) wacc_buildup present on ≥2.40.0 (applies:false with reason, or legal kd_source / Kd≥Rf / weight policy — coupon is not Kd; matches_buildup does not license a junk buildup).
Write S/registry/handoffs/5_valuation.md.
```

**5b — decision reopen (orchestrator lead, after Phase 2.5 complete; harness ≥ 2.14.0):** You are the **orchestrator lead** editing `decision.json` only — not a second valuer and not Agent 13. Do **not** rewrite `valuation_model.json` or compute scripts. Do **not** reset phase `2_parallel` / Agent 5 to pending. Do **not** spawn subagent id `5` in phase `2_5` (not allowed). Do **not** retag Agent 5 `execution` — keep `subagent` from the Phase 2 spawn; 5b is recorded on the orch row / `5b_decision_reopen` handoff. Read `S/registry/risk_bridge.json` and `S/registry/tsr_validation.json`. Update `duration.action` if stress or TSR flags change the verb. Set `reopened_after_stress: true`. If `tsr_validation.json` exists set `tsr_seen: true`. On harness ≥ 2.29.0 write `stress_bind` `{material, binding_scenario, haircut_pct, probability}`. **Material** = any stress scenario with `fair_value_haircut_pct ≥ 0.25` (or ≥25 if stored as percent points) **and** `probability ≥ 0.15`. If material: `duration.action` **must not** be `initiate` or `add` (pass/too_hard/hold/trim/sell/short). No override essay. Do not patch bear/base FV. On harness ≥ 2.32.0 if `tsr_validation.json` has `roc_vs_cost_of_capital=fail` and valuation `cheap_claim=franchise_mos` with `quality_bucket=above_wacc`, write `decision.roc_screen_rebuttal` ≥40 chars (mid-cycle NOPAT vs historical GAAP ROC). Do not rewrite `roic_identity`. This is **not** a risk red-team on whether p is honest (Agent 13 stays the evaluator). Optional handoff `S/registry/handoffs/5b_decision_reopen.md`.
...
Cover, in this order (harness ≥ 2.16.0 CIO page): (1) **quote** `duration.action` first plus one-sentence why from `decision.rationale` — do not invent a second verb; (2) cheap_claim class; (3) then fair value vs price + margin of safety as **context** (illustrative going-concern, not a bid, when action is pass/too_hard or cheap_claim ≠ franchise_mos). When `fair_value.decision_usefulness` is **low**, lead with the **cone + pass**, not the point FV. When `decision.stress_bind.material` is true, also lead with the cone + duration verb, not the point FV. (4) TA overlay side — not the duration book; (5) sector/market identity headers; latest-quarter headline; key risks (top 3); required inputs AS EXECUTED; data-quality notes; pointer to the other two reports. Do **not** lead with fair value vs price + MoS or a bull/base/bear cover verdict. Leave a one-line placeholder for the audit verdict — Phase 5 will fill it with **Audit PASS = process completeness (provenance/hooks), not an investment recommendation or buy list.** Do not put ATR share-count on this cover as a position size.
```
...
4-street. When street_estimates.json exists (harness ≥ 2.28.0): Street FY+1 is the **default** Y1 start (`street_baseline`). |delta_pct|>5% on street_baseline is major. `keep_independent_vs_street` is major. `independent_y1` without a resolved independence_gate is major. Destock-in-base without destock_this_print is **major**. Destock-in-base with destock_this_print is **not** a major. Destock-in-bear + Street-Y1 is **not** a major. Missing used_as:fy1_baseline on street_baseline is major. Unknown-basis Street EPS pasted as EBIT/NOPAT is major. Printed remaining-period box low undercut in base while Street sits inside the box is major. If `harness_version < 2.28`, stop and read ROOT/harness/law_history.md (2.18 required-Street Y1; 2.7–2.17 independent-then-calibrate) and grade that stamp against that law — do not apply 2.28 destock_this_print to a 2.18 session. Omit conservatism_dials or omit multi_method_reconciliation when both SOTP and DCF ran is machine FAIL on ≥2.7.0. `trust_guides_more` without a met-only / cash-quality split is a finding (beat is not a hit). two_quarter_rule that raises Y1 into worse FCF/AR/inventory without bear_only is a finding. two_quarter_rule that raises Y1 while destock conflict is live and FCF is ≥0 with inventory/AR down (WC release) is a finding (destock inverse; cannot raise).
...
4-wacc. On harness ≥ 2.40.0: `wacc_buildup` present (or applies:false with reason). Coupon / stated coupon as Kd is major. Pretax Kd below session Rf without `negative_rate_market` is major. `matches_buildup` with trough-only `current_market` weights while implied WACC is >200 bp above model is major. Target `wd` that copies trough `current_market_wd` without a non-price leverage hatch is major. Implied-gap rationale that is only “tape is cheap” / franchise MoS is major. Do not fail banks for missing industrial WACC. Do not treat a matched formula as conservative when ingredients are junk. See RESEARCH_AGENTS.md §10e.
