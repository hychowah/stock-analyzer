...
The map prompt's `ROOT` is this pin (live checkout **or** `pins/<semver>/`). Read **this file** and the rest of Mode A law **under `ROOT/`**. Run scripts as `python3 ROOT/scripts/<name>.py` (not checkout `scripts/` unless `ROOT` is the checkout). Write the session under `ARCHIVE_ROOT` / `S`. Do **not** open the interactive checkout's `harness/` to "update" the pin. Host MCP / `.grok` may come from the workspace cwd — that is tools, not law. Do not git commit from `ROOT`.
...
- **First action:** confirm the Yahoo listing with tools (yfinance / Yahoo MCP). Stamp `S/meta/run_manifest.json` `quote_symbol` (folder stays `TICKER`). Then `python3 ROOT/scripts/verify_listing.py --ticker T --date <session_key>`. Non-zero → `ROOT/scripts/abandon_session.py` then **STOP** before classification / Phase 0. Do not invent a company.
...
- Do not git commit.
- All other runbook rules still apply: preflight, `bind_library.py` before classify (and before 2b), `data/price_snapshot.json` freeze, Agent 5 including 5b, spawn-or-abandon, `check_session.py --full` before claiming done.
...
0. **Ticker check:** `python3 scripts/verify_ticker.py --ticker T`  
...
2. **Document library:** after listing stamp, run `python3 scripts/bind_library.py --ticker T --date D` **before** `sector_config`. If a latest annual `.txt` is in `S/data/raw_sec/`, line-range Item 1 / business before locking `primary_sector`. If bind is empty, classify from Yahoo + description and say so in `sector_config.signals`. Bind **refuses** completed sessions. Preflight `1_parallel` FAILs without `registry/library_bind.json`. Spawn 2b with `session_missing`. Read `harness/library.md` (you and 2b only). Do **not** run `harvest_library.py`. Year-reader paths = `S/data/raw_sec/*.txt` only.
3. Classify KPI sector from `harness/RESEARCH_AGENTS.md` **§5 first** (no scoring algorithm; modules do not classify and do not choose the engine). If you consult a `sector_*.md` detection list, it is diagnostic only. If `cyclical` is in play: name the sub-type or cousin, or show majority revenue **realized at** spot/index/posted producer prices; otherwise do not pick cyclical. Branded CPG / Consumer Defensive food with commodity-input or price-gap beta → `standard` (optional `is_also_growth`); seed protein/feed/HPAI in the brief, do not switch the lead module. Write `registry/sector_config.json` — `module_file` is a **string** (never JSON `null`; use `""` for standard). Use as-of **date** in JSON (`YYYY-MM-DD`), not necessarily the full session_key.
4. Write `registry/market_context.json` (intensity gate is load-bearing).
4b. Write `registry/classification.json` from `harness/modules/valuation_router.md` (§5c). This is the engine card. Agent 5 must not rewrite it.
5. Write `registry/research_brief.json` (new sessions) before Phase 0. Question (2) is destock **or** cycle **or** credit/underwriting **or** rate-base — do not force the word destock on a bank. Material commodity-input / protein-supply beta on a standard name must appear in `must_cover_risks`.
6. **Do not** open prior session folders (valuation, reports, handoffs, snapshots) for any phase inputs. Intra-session sharing only under current `S`. Phase 0 may run in parallel with `1_parallel` (2a/2b/2c). **1d still requires Phase 0 complete.**
...
7. **MUST spawn specialists** (isolated prompts) for Phase 0 swarm (one spawn per `registry/raw/phase0_*.json`), 2a∥2b∥2c, 2d, each 1c year-reader, 2e, 1d workers+merge, 1e `story` then `3p`, 4∥5∥12, 2.5 swarm (one spawn per `stress_*.json`), 6, 7∥8∥11, 13. Ritual: `record_spawn.py --event launch` → `spawn_subagent` → `--event return` (return without launch FAILs). Set `phase_status.agents[].execution=subagent`. Retry spawn without recording fail if the tool glitched. **`--event fail` (or `abandon_session.py`) only when you cannot launch and would otherwise work inline — then STOP.** Do **not** write that specialist’s artifacts yourself. Inline specialist work is a FAIL (`orchestrator_inline` forbidden; do not retag Agent 5 as `orchestrator_lead` at 5b). Orchestrator-lead work only: listing stamp, bind_library, sector/market/classification/brief, price_snapshot, phase_status, Phase 0/2.5 merge, 5b reopen.
...
11. **Phase 1c (after 2b; may overlap 2d):** list annuals (`packages/kd_research/annuals.py`). Spawn **one year-reader per annual** (isolated prompt; cleaned `.txt` path only — do not paste the filing). After `registry/raw/fdd_year_*.json` exist, run excerpt-in-source (`excerpt_check.py`) — **do not** run `--mode complete` yet (that gate needs FDD + `verify_rechecks`). Then spawn **2e merger** only. Then `preflight --phase 1c --mode complete`. Confirm `registry/filing_deep_dive.json` (plus `verify_rechecks`) before Agent 5. On ≥ 3.4.0, 1e `evidence_hooks` consume FDD before Agent 5; do not require `filing_deep_dive_hooks` on the model. On `< 3.4.0`, valuation must have non-empty `filing_deep_dive_hooks` before Phase 2.5. Legacy sessions without year-files: FDD alone still completes 1c.

11b. **Phase 1d (after Phase 0 + 1b, not 1c):** spawn `1d_rev` ∥ `1d_ind` ∥ `1d_ol` (gather only). Persist `registry/raw/oppath_*.json`. Then spawn `1d_merge` only. `preflight --phase 1d --mode complete`. Confirm `registry/operating_path_brief.json` before 1e. Workers must not write FV or 8-year paths. Do not average flatten vs destock. 1c may overlap 1d; 1e still needs FDD.
11c. **Phase 1e (after 0+1b+1c+1d):** spawn `story`, then `3p`. `preflight --phase 1e --mode complete` before Agent 5. Do not spawn Agent 5 on a FAIL 3P. Max two rewrite loops.

## Merges (Phase 0 / 2.5)

12. Persist each raw return under `registry/raw/` **before** merge.
13. Merge for coverage; spot-check ≥3 headline numbers; never invent merge numbers. Write swarm lead handoffs (`phase0_*.md`, `phase25_*.md`). Phase 2.5: merge shocks into `risk_bridge.json`, **then** `python -m packages.kd_research.stress_apply --ticker T --date D` (Apply stamps FV and reverse-stress; do not invent haircuts in the merge).
...
15b. **After Phase 2.5 is complete:** run RESEARCH_AGENTS.md §8 **5b** yourself (lead) — `stress_apply --skip-apply --bind-print`, write `stress_bind` on `decision.json` only, change duration only when `size_cap=0` forbids the verb. Then preflight `4_parallel` (reports wait on 2.5). Phase 3 charts wait on `valuation_model.json`, not 2.5 — spawn Agent 6 with `--phase 3` as soon as Agent 5 has written the model. Spawn Agent 4 / 12 with `--phase 2_parallel --subagent 4|12` (they do not wait on 1d or 1e). Spawn Agent 5 with `--subagent 5` (waits on 1e lock).
...
```
    ...
    Finalize **copies** scaffold-time `harness_version` + `harness_git_sha` / dirty into manifest + snapshot provenance — it does **not** recapture from live `harness/VERSION` / git. Confirm printed in CLI output.
