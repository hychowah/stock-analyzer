#!/usr/bin/env python3
"""Export finished research sessions into the comparison warehouse sqlite.

Path is catalog_sqlite_path (local home in production; archive/catalog in tests).
...
from packages.kd_research.ticker_lookup import listing_projection  # noqa: E402


def _payload_for_session(session: Path) -> dict[str, Any]:
    """Project an existing session from extract + on-disk snapshot. Does not write the session."""
    ...
    payload.update(listing_projection(session, ticker))
    return payload


def export_session(session: Path, conn) -> dict[str, Any]:
    """Insert-only projection into sqlite. Does not create or refresh prediction_snapshot.json."""
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        "(does not rewrite research sessions or prediction_snapshot.json)",
    )
    ap.add_argument(
        "--output-dir",
        type=Path,
    ...
    for s in sessions:
        try:
            results.append(export_session(s, conn))
