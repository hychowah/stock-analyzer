...
4-street. When street_estimates.json exists: independent_construction must be a company-evidence stack (guide/segments/run-rate/RPO), not “use consensus.” |delta_pct|>20% is a calibration note (not major by itself). Copying Street into base is major even if schema-valid. Copying Street into base is major even if schema-valid. Numeric base==street with a calibration hook is machine-allowed (landing near consensus); paste-without-needle is still Band 3 FAIL. Degraded transcripts that exile a printed IR/8-K outlook from base are major. Omit conservatism_dials or omit multi_method_reconciliation when both SOTP and DCF ran is machine FAIL on ≥2.7.0.
