...
Mode B consumes the dump (via Pin.workflow_spec / agent_prompt). Extra
entry files come from check_catalog.PATH_EXTRAS — do not copy
entry_checks() control flow.
"""
...
from packages.kd_research.check_catalog import PATH_EXTRAS
...
from packages.kd_research.phase_graph import DISPLAY_WRITES, PHASE_AGENTS
...
def build_workflow_spec(*, root: Path | None = None) -> dict[str, Any]:
    ...
    for pid, aids in PHASE_AGENTS:
        ...
        for extra in PATH_EXTRAS:
            if extra.phase == pid and parsed is not None and parsed >= extra.since:
                add_entry(
                    entry,
                    pid,
                    extra.rel,
                    required=extra.required,
                    since=_label(extra.since),
                )
        if pid == "2_parallel":
