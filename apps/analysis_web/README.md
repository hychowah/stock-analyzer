...
| `/api/portfolio/live-nav` | Mark current lots with last prints (not a catalog join). Aggregates + `quotes` (QuotePrint JSON) + per-lot `rows` (each row has `day_pl` and `contrib_pct` = day_pl / live_nav). Display math; FX is the statement Forex close |
| `/api/portfolio/mtm-interval` | One still for a window. Pass `period=` (`1w`, `1m`, `ytd`, `statement`) or `start=` and `end=` (YYYY-MM-DD), not both. `pl` is value(end) − value(start) + IB fill cash after start through end. Reads stored daily closes (no Yahoo on this GET). No frames, no bars. Heatmap window. User-initiated; not a poll |
| `/api/portfolio/mtm-path` | Daily reconstructed MTM frames. Pass `period=` (`1w`, `1m`, `ytd`, `statement`) or `start=` and `end=` (YYYY-MM-DD), not both. `pl` is value(t) − value(start) + IB fill cash after start; not the IB MTM file. Reads stored daily closes (no Yahoo on this GET). Play only. User-initiated; not a poll |
...
| `/api/price-history` | Daily closes for one `quote_listing` from `daily_closes.sqlite` (`symbol`, `range=1m\|3m\|6m\|1y\|2y\|5y\|max` is a display window, not the store key). Does not call Yahoo; run-detail HTML primes that listing |
...
Portfolio live marks (`data-quote-poll="0"` + `static/live_nav.js`): one GET `/api/portfolio/live-nav`; `quotes.js` paints Live/Downside from `quotes-applied` and does not fetch `/api/quotes` on that page. `heatmap.js` is the heatmap card (Live tiles from `holding-pl-applied`, a window fetches `/api/portfolio/mtm-interval`, Fill screen is a CSS class on `#heatmap-card`). Rows and heading commit together; loading is a status line on the current map. It does not write MTM bars. `mtm_play.js` fetches `/api/portfolio/mtm-path` and writes only the MTM bars.
