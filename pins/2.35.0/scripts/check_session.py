...
def main() -> int:
    ...
    if args.full and (session / "registry" / "street_estimates.json").is_file():
        check_file(
            session,
    ...
    from packages.kd_research.check_catalog import (  # noqa: WPS433
        WHEN_SESSION_CORE,
        WHEN_SESSION_FULL,
        run_catalog,
    )
    ...
    for status, check, detail in run_catalog(session, when):
        record(status, check, detail)
    if args.full:
        for step in (
