...
def _merge_provenance(
    ...
    """Keep scaffold identity (write-once). Fill holes only for legacy nulls."""
    out = dict(existing)
    ...
    for k in _PROVENANCE_IDENTITY_KEYS:
        if out.get(k) not in (None, ""):
            continue
        bundled = from_bundle.get(k)
        if bundled not in (None, ""):
            out[k] = bundled
    if out.get("harness_version") in (None, "") or out.get("harness_git_sha") in (None, ""):
    ...
    elif not out.get("harness_spec"):
        out["harness_spec"] = from_bundle.get("harness_spec") or "v2"
    return out
