...
On harness < 3.4.0 Agent 5 consumes the brief via operating_path_hooks.
On >= 3.4.0 phase 1e evidence_hooks own that consumption.
"""
...
from packages.kd_research.check_core import load_json as _core_load_json
...
def session_enforces_1d(session: Path) -> bool:
    ...
    if brief_path(session).is_file() or worker_files(session):
        return True
    return session_since(session, OPPATH_SINCE)


def load_json(path: Path) -> tuple[dict[str, Any] | None, str | None]:
    """Dict-only check loader. Missing message keeps the filename."""
    data, err = _core_load_json(path)
    if err:
        if err == "missing":
...
def check_operating_path_hooks(session: Path) -> list[tuple[str, str, str]]:
    ...
    from packages.kd_research.classification import CLASSIFICATION_SINCE

    if session_since(session, CLASSIFICATION_SINCE):
        return [("SKIPPED", "operating_path_hooks", "1e evidence_hooks own operating-path consumption")]
...
def check_oppath_entry(session: Path) -> list[tuple[str, str, str]]:
    """2_parallel entry: operating_path_hooks when 1d is in force and VM exists.

    Brief existence is a PhaseNode entry path, not this catalog row.
    """
    if not session_enforces_1d(session):
        return []
    if (session / "data" / "valuation_model.json").is_file():
        return check_operating_path_hooks(session)
    return []
