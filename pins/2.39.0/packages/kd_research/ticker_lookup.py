...
def snapshot_listing(session: Path) -> str | None:
...
def listing_projection(
    session: Path | None, ticker: str | None
) -> dict[str, str | None]:
    """Stamp + display listing for catalog export. Stamp has no ticker fallback."""
    stamp = quote_symbol_from_session(session) if session is not None else None
    snap = snapshot_listing(session) if session is not None else None
    ticker_u = str(ticker or "").strip().upper() or None
    if stamp:
        return {
            "quote_symbol": stamp,
            "quote_listing": stamp,
            "quote_listing_source": "stamp",
        }
    if snap:
        return {
            "quote_symbol": None,
            "quote_listing": snap,
            "quote_listing_source": "snapshot",
        }
    return {
        "quote_symbol": None,
        "quote_listing": ticker_u,
        "quote_listing_source": "ticker" if ticker_u else None,
    }
