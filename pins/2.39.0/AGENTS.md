...
Scaffold **new** `S` → listing stamp + `verify_listing` → `bind_library.py` (before `sector_config`) → classify + `market_context` → `research_brief` → Phase 0 ∥ 1_parallel → **preflight** before 1b / 1c / 1d / 2 / 2.5 / 4 / 5 → audit → `finalize_session` (copies scaffold stamps).  
