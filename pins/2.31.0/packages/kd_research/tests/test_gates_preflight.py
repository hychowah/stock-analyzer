...
class GatesTest(unittest.TestCase):
    ...
    def test_phase0_coverage_230_needs_three_raws(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            s = Path(td)
            _write(
                s / "meta/run_manifest.json",
                {"status": "scaffolded", "orchestrator_model": "grok-4.5", "harness_version": "2.30.0"},
            )
            _write(
            ...
            _write(
                s / "registry/raw/phase0_round1.json",
                {
                    ...
                    "findings": ["a"],
                    "sources": ["u"],
                    "downstream_relevance": "risk_candidate",
                },
            )
            rows = check_phase0_coverage(s)
            self.assertTrue(any(r[0] == "FAIL" and r[1] == "phase0_raw_count" for r in rows), rows)
...
class ResearchBriefSchemaTest(unittest.TestCase):
    def test_schema_file_exists_and_validates_example(self) -> None:
        schema_path = ROOT / "harness/schemas/research_brief.schema.json"
