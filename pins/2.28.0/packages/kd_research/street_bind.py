...
from packages.kd_research.gates import load_json, validate_hooks_list
...
DESTOCK_ANALOG_CLASSES = frozenset({"inventory_channel", "advertiser_budget"})
DIAL_KEYS = (
    "volume_vs_guide",
    "gaap_om_vs_guide",
    "sbc_in_fcff",
...
def session_is_gated_y1_runtime(session: Path) -> bool:
    """True when harness_version >= 2.28.0 (Street default; evidence-gated independent Y1)."""
    parsed = parse_semver(load_run_manifest_version(session))
    if parsed is None:
        return False
    return parsed >= STREET_GATED_Y1_SINCE


def destock_this_print(brief: dict[str, Any] | None) -> bool:
    """True when 1d says this print matches a destock analog class."""
    if not isinstance(brief, dict):
        return False
    analog = str(brief.get("analog_class") or "").strip().lower()
    flag = brief.get("current_print_is_destock")
    return analog in DESTOCK_ANALOG_CLASSES and flag is True
...
def _independence_gate_resolved(
    session: Path, bind: dict[str, Any]
) -> tuple[bool, str]:
    ...
    if gate not in LEGAL_INDEPENDENCE_GATES:
        return (
            False,
            f"independence_gate {gate!r} is not a legal 2.28 gate "
    ...
    if gate == "destock_this_print":
        ...
        if not destock_this_print(brief):
            return (
                False,
...
def _check_gated_y1_band(
    session: Path,
    bind: dict[str, Any],
    resp: str,
    expected: float,
) -> list[tuple[str, str, str]]:
    """2.28+ Y1 band: 5% FAIL on street_baseline; independent_y1 needs a resolved gate."""
    out: list[tuple[str, str, str]] = []
    abs_d = abs(expected)
    if resp == "street_unusable":
        return [("PASS", "street_bind.y1_band", "street_unusable; no 5% identity")]
    if resp == "independent_y1":
        ok, detail = _independence_gate_resolved(session, bind)
        if not ok:
            ...
            if len(div.strip()) < DIVERGENCE_MIN_LEN:
                out.append(
                    (
                        ...
                        "|delta_pct| > 20% on independent_y1 requires divergence_rationale ≥40 chars",
                    )
                )
            else:
                out.append(("PASS", "street_bind.delta_calibration", "divergence_rationale present"))
        elif abs_d > Y1_BAND:
            out.append(
                (
                    "WARN",
                    "street_bind.y1_band",
                    f"|delta_pct| {expected:.4f} > 5% on independent_y1 — document the gate overlay",
                )
            )
        elif abs_d > Y1_WARN_BAND:
            out.append(
                (
                    "WARN",
                    "street_bind.y1_band",
        ...
        return out
    # Default / street_baseline / reopen_path / empty
    if abs_d > Y1_BAND:
        out.append(
            (
                "FAIL",
                "street_bind.y1_band",
                f"|delta_pct| {expected:.4f} > 5% vs Street FY+1 — street_baseline must "
                "start from Street, or set response=independent_y1 with a resolved gate, "
                "or street_unusable",
            )
        )
    elif abs_d > Y1_WARN_BAND:
        ...
        out.append(("PASS", "street_bind.y1_band", f"|delta_pct| {expected:.4f} inside 5%"))
    return out
...
def check_street_bind(session: Path) -> list[tuple[str, str, str]]:
    ...
    gated = session_is_gated_y1_runtime(session)
    rationale = _y1_construction_rationale(bind)
    construction_id = (
        "street_bind.y1_construction" if gated else "street_bind.independent_construction"
    )
    ...
    if street_col is not None and abs(street_col) > 1e-12:
        ...
        if y1_runtime:
            ...
            if gated:
                out.extend(_check_gated_y1_band(session, bind, resp, expected))
            elif resp != "street_unusable" and abs(expected) > Y1_BAND:
                out.append(
                    (
                        "FAIL",
                        "street_bind.y1_band",
            ...
            elif (not gated) and resp != "street_unusable" and abs(expected) > Y1_WARN_BAND:
                out.append(
                    (
                        "WARN",
                        "street_bind.y1_band",
            ...
            if (not gated) and abs(expected) > DELTA_THRESHOLD:
                div = str(bind.get("divergence_rationale") or "")
    ...
    if isinstance(hooks, list) and hooks:
        ...
        for h in hooks:
            ...
            if any(n in a for n in PATH_COPY_NEEDLES):
                copy_hits.append(a)
        if y1_runtime:
            independent = gated and resp_early == "independent_y1"
