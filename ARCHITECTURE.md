...
| `/portfolio/histories/{id}` | First paint is the paper book (lots that day, including names later sold on this copy). Marks and the NAV path load as their own GETs — copy does not wait on Yahoo. Sell takes a visible quantity. Header Δ is the last path point. Actual is this copy, not a live IB walk. Does not call Live NAV. Works if the IB file is later missing. |
