"""Rebuild archive/catalog JSON indexes from disk (library).

CLI wrapper: ``python3 scripts/rebuild_catalog.py``.
"""
...
from packages.kd_research.registry_io import atomic_write_text, load_json
...
def _row_for_session(ticker: str, session_key: str, path: Path) -> dict[str, Any]:
    ...
    outcomes_dir = outcomes_root(path) / ticker / session_key
    if not outcomes_dir.is_dir():
        # also try plain date for outcomes keyed historically by date
        outcomes_dir = outcomes_root(path) / ticker / session_date
    ...
    return {
        ...
        **listing_projection(path, ticker),
    }


def rebuild(*, output_dir: Path | str | None = None) -> dict[str, Any]:
    dirs = ensure_archive_tree(output_dir)
    sessions = iter_research_sessions(output_dir)
...
def patch_run_into_catalog(
    ...
    If indexes are missing, falls back to full rebuild of that archive.
    """
    dirs = ensure_archive_tree(path)
    ...
    if not runs_path.is_file() or not tickers_path.is_file():
        return rebuild(output_dir=archive)
...
def rebuild_from_sqlite(*, output_dir: Path | str | None = None) -> dict[str, Any]:
    ...
    from packages.kd_research.compare_db import db_path
