...
Load current policy: ROOT/harness/RESEARCH_AGENTS.md §10c (Street/Y1 + destock), §10d (ROIC), §10e (WACC). Do not restate those sections here. Do not load ROOT/harness/law_history.md (Agent 13 only when harness_version < 3.0.1). On harness ≥ 3.0.0: load ROOT/harness/modules/valuation_router.md then valuation_checks.md. **Lock then compute:** write S/registry/narrative_bind.json (status=locked) BEFORE valuation_model.json. Do not restory while computing.
...
4-wacc. Grade `wacc_buildup` against §10e.
