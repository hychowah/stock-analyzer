...
## 10c. Street FY+1 is base Y1 (current, harness ≥ 2.18.0)

**Next-year Street is the Y1 baseline.** Vendor FY+1 revenue consensus is fairly accurate on that horizon. Agent 5 does **not** invent an independent destock or duration FY+1 and treat Street as a footnote. Street **price targets** stay out of the DCF.

This section is **current law**. Do not apply 2.7–2.17 calibration-era destock/Street sentences here. Sessions stamped 2.7–2.17 keep independent-then-calibrate Y1 and (from 2.12) destock-in-base — §13 bounded rows, not this section.
...
3. **Agent 5** sets **base Y1 revenue = Street FY+1** when that value is numeric and usable. Write `street_bind`: `guide` × `street` × `base` and `delta_pct = (base − street) / street`. `street` must **match** `street_estimates.json` FY+1 (`response=street_unusable` to skip). `independent_construction.rationale` must show Street as the Y1 start plus any overlay. `street_hooks[]` **must** include `used_as:fy1_baseline` (or `used_as:revenue_path` / `street_mean`). Do not average destock with Street into one CAGR called base. Intra-year Street 0q/+1q when present must be consistent with FY+1. FY+2 Street is a **growth reference**, not a mandatory paste.
4. **Band (machine ≥ 2.18.0):** `|delta_pct| > 0.05` is FAIL unless `response=street_unusable`. `|delta_pct| > 0.03` is WARN. `keep_independent_vs_street` is illegal when Street is numeric. Remaining-period **low** of a live unprinted company box is a floor for that **same period** in base (Q3 box vs Q3 path, never Q3 vs FY Y1). A miss of a guide that has not printed belongs in **bear**.
5. **Profit:** if Street EBIT/OI is fetched, base Y1 EBIT starts there. Else if Street EPS is numeric and `eps_basis` is `gaap` or `non_gaap`, start from EPS × session share count and write an EPS→NI→EBIT bridge. If `eps_basis=unknown` and no Street EBIT, Street EPS is calibration only; profit start = company OI/expense box when present, else widen the cone.
6. **Allowed deviations:** bear path (destock analog, recession, legal, capex overshoot); bull above Street; named `volume_vs_guide` / `gaap_om_vs_guide` with `applies_in=bear_only`; `street_unusable` (fetch failed, `n_revenue` < 5, wrong FY map, `unavailable=true`) — then company box / H1+remaining and **widen the cone**. Native-KPI names (`banking` / `insurance` / `reit`) bind the native Street KPI, not a forced revenue DCF.
7. **Destock analog (harness ≥ 2.18.0):** while Street is usable, destock/quality-reset belongs in **bear**, not base. `4d` does **not** win `4e`. Destock-in-base is legal only if Street is unusable **and** the analog matches this print (inventory/channel WC-release or advertiser-budget with price/ad down — not capex-crushed IR FCF). 1d writes `analog_class` (`inventory_channel` | `advertiser_budget` | `none`) and `current_print_is_destock`.
8. **Conservatism stacking:** on new runtime, valuation **must** write `conservatism_dials[]` with keys `volume_vs_guide`, `gaap_om_vs_guide`, `sbc_in_fcff`, `wacc_vs_buildup` (`applies_in` base | bear_only | none). Omitting the array is FAIL (silent stacking). `stacking_justification` when ≥3 apply in base. Economic SBC-as-cash remains allowed; stacking it with a haircut-the-guide volume path as “base” is FAIL-quality. On ≥ 2.18.0, `volume_vs_guide` **and** `sbc_in_fcff` both in base while Y1 is outside the Street 5% band is FAIL.
9. **SOTP vs DCF:** when both run (model.name / methods / assumption keys), write `multi_method_reconciliation` — omit is FAIL on new runtime. If `|cross − primary| / primary > 0.40`, reopen the Y1 path or explain why the gap is real (cash vs earnings, segments). Do not keep a 40%+ gap with `why_primary_wins` theater.

Legacy sessions without `street_estimates.json` and harness < 2.7.0 → SKIPPED. Harness 2.7–2.17 → old independent-then-calibrate machine.
...
| Harness ≥ 2.12.0 and < 2.18.0: destock conflict of any status cannot park destock in bear while duration stays in base (destock-in-base / DU=low / pass\|too_hard); destock-inverse two-quarter raise is WARN. On ≥ 2.18.0 this invert: Street usable → destock analog in **bear**, destock-in-base FAIL (DU=low is not an escape). Legacy / <2.12.0 → SKIPPED | `[machine]` `check_session --full` |
