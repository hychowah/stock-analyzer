...
2.28.0–2.44.x and >= 3.0.1: Street FY+1 is the *default* Y1 forecast
(street_baseline; |delta|>5% FAIL). independent_y1 is legal when a named
evidence gate resolves. Destock-in-base is legal only when destock_this_print.
keep_independent_vs_street remains illegal.

3.0.0 only: independent_y1 is the default (no gate). 3.0.1 restores Street as
the default forecast. Named exits and story_fail tightness live on
StreetY1Policy (GATES_228 vs GATES_300; story_fail_requires_will_own).
...
def _independence_gate_resolved(
    ...
    if gate == "story_fail":
        ...
        if policy.story_fail_requires_will_own:
            return False, "story_fail requires narrative_bind.street_restory.will_own=false"
    ...
    if gate == "ttc_midcycle":
        ...
        if isinstance(sc, dict):
            sector = str(sc.get("primary_sector") or "").strip().lower()
        overlays: list[str] = []
        nb, _nb_err = load_json(session / "registry" / "narrative_bind.json")
        if isinstance(nb, dict) and isinstance(nb.get("overlays"), list):
            overlays = [str(x).strip().lower() for x in nb["overlays"]]
        if sector != "cyclical" and "cyclical" not in overlays:
            return (
                False,
                "ttc_midcycle requires primary_sector=cyclical or cyclical overlay",
            )
...
def _apply_y1_policy(
    ...
    if policy.independent_y1_ok and resp == "independent_y1":
        ok, detail = _independence_gate_resolved(session, bind, policy)
        if not ok:
