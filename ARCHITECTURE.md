...
| `/portfolio/histories/{id}` | First paint is the paper book (lots and cash that day; names later sold on this copy are a page badge). Marks and the NAV path load as their own GETs — copy does not wait on Yahoo. Header cash is today. Sell takes a visible quantity. Header Δ is the last path point. Actual is this copy, not a live IB walk. Does not call Live NAV. Works if the IB file is later missing. |
