...
> **KPI / stress library — not the engine.** Playbook comes from `valuation_router.md` (`young_startup` or `mature_operating` + `growth_assets`). `fair_value.base` is a DCF. ARR, EV/Revenue, and exit multiples belong in `valuation_model.pricing` (price column), never as terminal value or the decision number.
> **Module Purpose**: Unit economics, path-to-profitability, SBC, and stress seeds for high-growth / pre-profit names.
...
- If invested capital is positive, `harness/RESEARCH_AGENTS.md` §10d applies (`applies:false` is banks/insurance/negative-IC pre-profit — not “we prefer ARR”; REIT keeps ROIC/WACC on ≥ 3.1.0).
...
- **Only after profitability is achieved** (never grow a negative EBIT base)
- Intrinsic TV is Gordon / stable growth after the firm is mature (`g = RR × ROC`), or liquidation. **Exit multiple is price** — write it on `pricing`, not `fair_value.base`
- Terminal value applied at the year FCF is sustainably positive (typically Year 7–10)
- Growth rate in perpetuity: funded, and \(g_n \le\) rf in that currency (not a pasted 3–4%)
...
### Model D: ARR Multiple (SaaS-Specific) — **price column**

Pricing check for SaaS, not `fair_value.base`. Write on `valuation_model.pricing`.
...
|-------------------|---------------------------|-----------|
| **DCF (FCF-based)** | Extended DCF (path to profitability) as **value**; ARR multiple as **price** | FCF is negative for years; value comes from future profitability. Model the journey to positive FCF explicitly. ARR multiple is `pricing`, not `fair_value.base`. |
...
**Do not replace the DCF terminal value with an ARR multiple.** An ARR-based "terminal value" is a **price** cross-check, not intrinsic TV — it belongs in `valuation_model.pricing`, never in `terminal_consistency` or `fair_value.base` (`valuation_checks.md`: exit multiple / ARR / NAV are price). If the DCF looks low, fix the **value** path (fade terminal growth explicitly, extend the forecast, or revisit the reinvestment/margin assumptions) and then compare:
```
DCF value (Gordon / stable growth)         = $244M   (value)
Price cross-check: ARR_Y7 × ARR multiple   = $265M × 6x   (price column)
```
Report the two ledgers separately. Never average them and never let the multiple write `fair_value.base`.

**Conclusion (illustrative): DCF value ≈ $244M; ARR price cross-check ≈ $1.6B EV. The gap is a statement about the sector's pricing, not permission to relabel the multiple as value.**
...
### 11.1 Summary Valuation Cross-Check — **price**, not value

Everything in this section is an EV/ARR **price** cross-check. None of it writes `fair_value.base`; the intrinsic value is the DCF (explicit FCF path). Do not average these into the DCF, and do not lift one row as the value.

| Price cross-check | EV ($M) | EV/ARR | Key Driver |
|----------|---------|--------|------------|
| Extended DCF (explicit FCF path) | $650M | 6.5x | Path to profitability |
...
| **Price cross-check range** | **$600–650M** | **6.0–6.5x** | **Consensus range** |

**Price cross-check range: $550M – $700M EV (5.5x – 7.0x ARR)**
...
Terminal Value (perpetuity) = FCF × (1 + g) / (WACC - g)   # value; g funded
# Exit multiple / EV/ARR = pricing column, not fair_value.base
```
...
- **Fix**: Use extended DCF with explicit path to profitability. ARR multiple is the **price** column, not a substitute for value.
