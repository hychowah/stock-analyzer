...
from scripts.kd_research.annuals import is_annual_form, list_annuals_from_filenames  # noqa: E402
...
class PathsTests(unittest.TestCase):
    def test_library_root_and_not_a_session(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (tlib / "filings").mkdir()
            rows = iter_research_sessions(root, include_legacy=False)
            self.assertEqual(rows, [])

    def test_library_reserved(self):
        self.assertIn("library", ROOT_RESERVED_NAMES)
