...
from packages.kd_research.consumption import check_market_context
...
class MarketContextCheckTests(unittest.TestCase):
    ...
    def test_absent_market_context_is_skipped_not_fail(self):
        ...
        rows = check_market_context(session)
        statuses = [s for s, c, _ in rows if c == "market_context"]
        self.assertEqual(statuses, ["SKIPPED"])
        self.assertFalse(any(s == "FAIL" for s, _, _ in rows))
    ...
    def test_valid_low_intensity_with_noted_only_hook_passes(self):
        ...
        self.assertTrue(any(s == "PASS" and c == "market_context_hooks" for s, c, _ in rows))
    ...
    def test_high_intensity_all_noted_only_fails(self):
        ...
        rows = check_market_context(session)
        self.assertTrue(
            any(s == "FAIL" and "intensity" in c for s, c, _ in rows),
            msg=rows,
        )
    ...
    def test_present_without_hooks_fails(self):
        ...
        rows = check_market_context(session)
        hook_fails = [r for r in rows if r[0] == "FAIL" and "market_context_hooks" in r[1]]
        self.assertTrue(hook_fails, msg=rows)
    ...
    def test_short_rationale_fails(self):
        ...
        rows = check_market_context(session)
        self.assertTrue(
            any(s == "FAIL" and "rationale" in c for s, c, _ in rows),
            msg=rows,
        )
