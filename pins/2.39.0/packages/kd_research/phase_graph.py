...
class PhaseNode:
    """One research phase. Priors are direct edges, not a linear walk."""
...
# Live DAG. Overlays: <2.30 add 0 as prior of 1_parallel; <2.6 omit 1d.
PHASE_GRAPH: tuple[PhaseNode, ...] = (
    _n("orch", (), ("orchestrator",), spawn_ids=(), display_writes={
        "orchestrator": (
...
PHASE_ENTRY_REQUIRED: dict[str, list[str]] = {
    n.phase_id: list(n.entry_required) for n in PHASE_GRAPH
}
PHASE_ENTRY_OPTIONAL: dict[str, list[str]] = {
    n.phase_id: list(n.entry_optional) for n in PHASE_GRAPH if n.entry_optional
}
PHASE_COMPLETE_PATHS: dict[str, list[str]] = {
    n.phase_id: list(n.complete_paths) for n in PHASE_GRAPH if n.complete_paths
}
...
def check_phase_graph_entry(
    ...
    *,
    ...
    if isinstance(current, str) and node is not None:
        if current == phase_id or current in node.priors:
            results.append(
                (
                    "PASS",
