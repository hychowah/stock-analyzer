...
Stamp `narrative_bind.iv_playbook` to the playbook id (harness ≥ 3.1.0) and `narrative_bind.buyer` (harness ≥ 3.3.0). Overlays go in `narrative_bind.overlays`. Lock the story (`registry/narrative_bind.json`) then compute. Checks: `harness/modules/valuation_checks.md`. Optional `valuation_model.pricing` is a labeled **price** column (NAV, ARR, TTC×EBIT, exit multiple). It never writes `fair_value.base`.
