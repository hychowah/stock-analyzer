...
## 10c. Street FY+1 is the default Y1 start (current, harness ≥ 2.28.0)
...
This section is **current law**. Sessions stamped 2.18.0–2.27.x keep required Street Y1 (`|delta|>5%` FAIL; destock-in-base FAIL while Street usable) — §13 bounded row, not this section. Sessions stamped 2.7–2.17 keep independent-then-calibrate Y1. Historical destock/Street captions: `harness/law_history.md` (Agent 13 loads only when `harness_version < 2.28`).
...
7. **Destock analog (harness ≥ 2.28.0):** destock-in-base is **legal** only when `destock_this_print`. Destock-in-base remains **FAIL** while Street is usable and this print is **not** destock. Destock analog in **bear** is always legal. Mid-cycle / TTC construction stays an ROIC identity control — it does **not** license Y1 to leave Street.
8. **Conservatism stacking:** on new runtime, valuation **must** write `conservatism_dials[]` with keys `volume_vs_guide`, `gaap_om_vs_guide`, `sbc_in_fcff`, `wacc_vs_buildup` (`applies_in` base | bear_only | none). Omitting the array is FAIL. `stacking_justification` when ≥3 apply in base. On ≥ 2.18.0, `volume_vs_guide` **and** `sbc_in_fcff` both in base while Y1 is outside the Street 5% band is FAIL.
9. **SOTP vs DCF:** when both run, write `multi_method_reconciliation` — omit is FAIL on new runtime. If `|cross − primary| / primary > 0.40`, reopen the Y1 path or explain why the gap is real. If `data/compute/valuation_result.json` exposes Y1 revenue, `street_bind.base` must match it.

Legacy sessions without `street_estimates.json` and harness < 2.7.0 → SKIPPED. Harness 2.7–2.17 → old independent-then-calibrate machine. Harness 2.18–2.27 → required Street Y1 / destock-in-bear.
...
| New-runtime `street_estimates.json` + valuation: `street_bind` construction; `delta_pct` identity; on 2.7–2.17 `|delta|>20%` is a calibration WARN and path-copy is FAIL; on ≥ 2.18.0 and < 2.28.0 `|delta|>5%` is FAIL unless `street_unusable`, `used_as:fy1_baseline` required, `keep_independent_vs_street` illegal; on ≥ 2.28.0 `street_baseline` keeps the 5% FAIL, `independent_y1` needs a resolved evidence gate. Absent on legacy → SKIPPED | `[machine]` `check_session --full` + `2_parallel` complete |
...
| Harness ≥ 2.10.0: `registry/decision.json` duration.action including `pass`/`too_hard`; `initiate`/`add` illegal when `decision_usefulness=low` or wide cone; technical `side=pass` legal (no forced long); TA `side=pass` legal (no forced long). Duration pass + TA long is legal (isolated specialists). README must quote `duration.action`. Catalog projects `decision_action` + risk-bridge `monitoring_trigger`s. Absent on legacy → SKIPPED | `[machine]` `check_session --full` |
| Harness ≥ 2.11.0: TV share >60% with Y8 growth ≥8% requires extend/switch; related-party ≥20% cannot stay intensity=low; `research_brief.mode=update` requires facts-only earning_power_changelog (no prior FV/MoS/WACC). Destock/Street Y1 machines are version-banded (2.12 and 2.18 rows; §10c). Legacy → SKIPPED | `[machine]` `check_session --full` |
| Harness ≥ 2.11.0 and < 2.18.0: unresolved flatten-vs-destock cannot be duration-in-base unless destock is in base, DU=low, or duration=pass; Street \|delta\|>20% is a calibration WARN (copying Street still FAIL). Legacy / ≥2.18.0 → 2.12/2.18 rows | `[machine]` `check_session --full` |
| Harness ≥ 2.12.0 and < 2.18.0: destock conflict of any status cannot park destock in bear while duration stays in base (destock-in-base / DU=low / pass\|too_hard); destock-inverse two-quarter raise is WARN. On ≥ 2.18.0 and < 2.28.0 this invert: Street usable → destock analog in **bear**, destock-in-base FAIL (DU=low is not an escape). On ≥ 2.28.0 see the 2.28 row. Legacy / <2.12.0 → SKIPPED | `[machine]` `check_session --full` |
| Harness ≥ 2.13.0: `roic_identity.applies:true` requires `mid_cycle_construction`; `above_wacc` / `franchise_mos` need a ≥2-year ttc_cycle|multi_year_avg window (last_year/peak never license). TTC &lt; WACC remains non-FAIL. Legacy → SKIPPED | `[machine]` `check_session --full` |
...
| Harness ≥ 2.18.0 and < 2.28.0: Street FY+1 is required base Y1 (`used_as:fy1_baseline`; \|delta\|>5% FAIL); destock analog in bear while Street usable; destock-in-base FAIL. Legacy 2.7–2.17 Street/destock machines unchanged. | `[machine]` `check_session --full` |
| Harness ≥ 2.28.0: Street FY+1 is the **default** Y1 start (`street_baseline`; \|delta\|>5% FAIL). `independent_y1` is legal with a resolved `independence_gate` (`destock_this_print` \| `definition_mismatch` \| `native_kpi` \| `street_unusable`). Destock-in-base legal only when `destock_this_print`. `ttc_midcycle` is not a Y1 license. `keep_independent_vs_street` illegal. | `[machine]` `check_session --full` |
| Harness ≥ 2.29.0: after `risk_bridge.json` exists, `decision.stress_bind` is required. Material stress (haircut≥25% and p≥0.15) forbids `initiate`/`add`. No DCF rewrite. No override hatch. `narrative_only` illegal on material scenarios. Legacy → SKIPPED | `[machine]` `check_session --full` + `4_parallel` preflight |
| Harness ≥ 2.30.0: bind_library before sector classify; Phase 0 default 3 rounds (4 if deep/medium/high); `1_parallel` does not wait on Phase 0; 1d still does. Phase 0 complete needs ≥3 (or ≥4) raws. Legacy min 1 raw. | `[machine]` `preflight` + `check_session --full` |
| Harness ≥ 2.31.0: extracted FDD material notes (SBC/legal/RP/debt or scorecard) need use-or-reject hooks; 2d consensus beat/miss cites `street_estimates.json`; 1d_ind has `background_round_ids` or `named_gaps`; legal $ in 2.5 cites FDD or unknown. Legacy → SKIPPED | `[machine]` `check_session --full` |
| Harness ≥ 2.32.0: Phase 2 stays `4 ∥ 5 ∥ 12`. Agent 5 must not wait for TSR. `franchise_mos` + `above_wacc` + Agent 12 `roc=fail` requires `decision.roc_screen_rebuttal` ≥40 chars (FAIL, not WARN). Agent 4 must not cite `tsr_validation`. Legacy WARN-only. | `[machine]` `check_session --full` |
