...
**Document library before classify (harness ≥ 2.30.0):** after listing stamp + `verify_listing.py`, run `bind_library.py` **before** `sector_config`. If a latest annual `.txt` is in `S/data/raw_sec/`, line-range Item 1 / business description before locking `primary_sector`. If bind is empty, classify as today and say so in `sector_config.signals`.
