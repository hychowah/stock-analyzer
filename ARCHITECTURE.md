...
flowchart TD
  ...
  Stamp --> Classify[Classify KPI sector, engine card, brief]
  ...
  P1b --> P1d
  P0 --> Story[1e story + 3P lock]
  P1b --> Story
  P1c --> Story
  P1d --> Story
  Story --> Val5[Agent 5 valuation]
  Classify --> Tech[Agent 4 technical]
  Classify --> Tsr[Agent 12 TSR]
...
Check ticker (quote or listings) → scaffold (skeleton, not blank) → write `quote_symbol` + verify listing + bind library → classify KPI sector, write the engine card (`classification.json`), then the brief. Phase 0 runs in parallel with Phase 1. Then 1b latest quarter ∥ 1c filing deep dive (file gates: 1b needs financials+filings, 1c needs filings; neither waits on news). Then 1d operating path (waits on Phase 0 **and** 1b, not 1c). Then 1e story + 3P critic lock (waits on 0+1b+1c+1d). Technical and TSR may start without 1d/1e. Agent 5 waits on the 1e lock and does not reclassify. Then stress/risk (workers propose shocks; `python -m packages.kd_research.stress_apply` reruns Agent 5’s `fair_value_under` — not a second model). Then 5b writes `stress_bind` (size cap / expected loss) without rewriting fair value. Charts start after valuation_model. Reports start after stress (they copy the **Unstressed vs stressed** card). Audit waits on reports, not charts. Then finalize.
