...
Write `valuation_model.wacc_buildup` on the same compute script as the DCF. `applies:false` with `not_applicable_reason` ≥40 chars + `native_analog` for banking (`roe_vs_ke`), insurance, REIT NAV/AFFO — copy §10d.5; do not invent an industrial WACC to satisfy the object.
...
**Current machine gates only.** Version-banded Street/destock/graph history is `harness/law_history.md` (Agent 13 when `harness_version < 3.0.1`). Python still SKIPPED-gates older stamps; live operators do not need those rows. Damodaran identities (`narrative_bind`, bank engine, growth triad, truncation-writes-base): `packages.kd_research.damodaran_gates` on ≥ 3.0.0.
