...
- Phase 2.5: fixed **≥5 scenarios** remains machine-checked; region/gov scenario must be material on deep names. Harness ≥ 2.44.0: workers propose replacement shocks on `shock_surface.dials`; merge then `stress_apply`; haircuts from `fair_value_under`; 5b binds `size_cap` / expected loss; reports copy `format_stress_card`. The quota was not dropped.  
