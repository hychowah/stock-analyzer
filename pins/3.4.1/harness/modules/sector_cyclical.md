...
> **KPI / stress library — not the engine.** Host playbook from `valuation_router.md` plus `cyclical` overlay. Value is a host DCF with cycle-average **margin × current revenues**. TTC EV/EBIT, NAV, and replacement cost are the **price** column (`valuation_model.pricing`), never `fair_value.base`. Peak/trough year is not the DCF base. Do not skip DCF at trough.

## Executive Summary

Cyclical industries represent a unique analytical challenge: standard valuation frameworks fail because earnings are mean-reverting, peak valuations signal danger, and trough valuations signal opportunity. This module provides cycle facts, TTC KPIs, and stress seeds for commodity producers, manufacturing cyclicals, shipping, and semiconductors.

**Core Principle:** For cyclicals, the cycle IS the investment thesis. A steel company at 3x P/E is expensive; at 30x P/E it may be cheap. That is a **pricing** inversion. Intrinsic value still uses mid-cycle earnings in a DCF.
...
### Decision Flow: Value vs price

```
VALUE (fair_value.base) — all cyclical types:
    Host DCF + cyclical overlay
    Base year: cycle-average operating margin × current revenues
    Never peak-year or trough-year EBIT as the unadjusted base
    TV: Gordon on mid-cycle FCFF (g funded; mines may use g=0), not an exit multiple

PRICE (valuation_model.pricing) — do not write these as fair_value.base:
    commodity_producer: NAV at long-run commodity price; TTC EV/EBITDA
    manufacturing_cyclical: TTC EV/EBIT; replacement cost
    shipping: NAV (vessel values); TTC earnings
    semiconductor: TTC EV/EBITDA; P/B of fabs
    trough: liquidation / replacement cost as a floor check
    P/E at trough or peak is a pricing ritual, not value
```
