...
| Variable | Example |
|---|---|
| `TICKER` | `JPM` (session folder / catalog identity) |
| `YAHOO_QUOTE_SYMBOL` | `JPM` (Yahoo listing; may differ, e.g. `ADYEN` vs `ADYEN.AS`) |
| `DATE` | `2026-07-25` |
...
- **Yahoo listing**: yfinance / Yahoo / price MCP fetches use `S/meta/run_manifest.json` `quote_symbol` (`YAHOO_QUOTE_SYMBOL`). It may differ from `TICKER` (archive folder). Do not invent an exchange suffix. Orchestrator injects both.
...
- **Decision-grade return contract** (every agent):
  ...
  - **Swarm workers (Phase 0 / 2.5)**: return **JSON only** with mandatory decision fields (see templates). Prefer **signal density**: 3–6 specific, sourced findings beat 20 generic industry platitudes. Do **not** write `S/data/raw_sec` or transcripts (Phase 1). Returns carry **paths + short excerpts**, not uncapped 10-K prose.
...
**HARD RULE — spawn or abandon:** Every specialist (Phase 0 swarm, 2a/2b/2c, 2d, each year-reader, 2e, 1d workers+merge, 1e story then 3p, 4/5/12, 2.5 swarm, 6, 7/8/11, 13) **must** be launched with `spawn_subagent`. You do **not** write their artifacts. Before each spawn:
...
**New-run start (mandatory):** When the user asks to research ticker T (and does not name an existing `session_key` to resume), **existence-check first** (`python3 scripts/verify_ticker.py --ticker T`; `scaffold_session.py` does this then writes). Live Yahoo quote **or** Yahoo search listings → continue with the typed session ticker (`quote_symbol` null). No quote and no listings: **abort** — do not scaffold, do not invent a company. After scaffold, **confirm the Yahoo listing with tools**, stamp `quote_symbol`, then `python3 scripts/verify_listing.py --ticker T --date <session_key>` (do not rename the folder). Non-zero or not a real issuer: abandon and STOP before classification / Phase 0. Then follow **New-run sequence** (bind before classify) and work under that `S`. Always pass **`--orchestrator-model <id>`** (your actual model id, e.g. `grok-4.5`) so `meta/run_manifest.json` records it **before Phase 0** — never invent or guess the model id at finalize after a long context. Optional `--subagent-model` defaults to the same id. Preflight FAILs if missing. **Forbidden:** listing `archive/research/T/`, reading yesterday’s reports/registry, or deciding whether a prior run is “usable” before starting. That is cross-session contamination. Same-day re-runs get a new `session_key` (`date` or auto `date__rN`); keep `S` pointed only at the current folder. Do not inject any prior session paths into subagent prompts. `--skip-ticker-check` is tests/offline only.

**New-run sequence (one list):** listing stamp + `verify_listing.py` → `bind_library.py` **before** `sector_config` → classify KPI sector (§5 first; modules do not classify) → market_context → `classification.json` (§5c engine from `valuation_router.md`) → `research_brief.json` → Phase 0 ∥ 1_parallel. After 1d+FDD: 1e story then 3p; then Agent 5. If a latest annual `.txt` is in `S/data/raw_sec/`, line-range Item 1 / business description before locking `primary_sector`. If bind is empty, classify as today and say so in `sector_config.signals`. Bind **refuses** a completed session (`prediction_snapshot` or finalized `run_manifest`) — never re-bind finished history. Bind must still finish before spawning 2b. Read `ROOT/harness/library.md` (you and Agent 2b only; do **not** paste it or the live library path into other subagents). Inject `session_missing` from bind/index into the **2b** prompt only. Year-reader paths must be `S/data/raw_sec/*.txt`. Do **not** run `harvest_library.py` from a new research run.
...
**Research brief (new sessions):** after bind + sector_config + market_context, write `research_brief.json`. `must_answer_questions` still min 3. Include at least one question each on (1) the industry game / who sets price, (2) destock vs demand **or** cycle **or** credit/underwriting **or** rate-base (identity-native — do not force the word destock on a bank), (3) whether you would trust retained earnings. Schema minItems stays 3; these seeds are coverage, not extra schema required keys.
...
`AgentSwarm`, subagent_type `explore`, one item per round. Default rounds: (1) company & business model (primary-first if `S/data/raw_sec` annuals exist — short-broad-then-narrow, not a TAM/moat essay), (2) open `must_answer_questions` not already answered by sector_config / market_context, (3) bear case & `risk_candidate` list. Add round (4) only if `module_file` is non-empty **or** `research_depth=deep` **or** intensity medium/high: 2–3 sector-module questions plus control/ownership/FX when intensity warrants. Do **not** spawn standalone TAM, moat, industry, or management encyclopedia rounds — `1d_ind` / `2e` / `1d_rev` own those. Round (1) extra keys when a moat is claimed: `mechanism` (switching_cost|network|cost|scale|brand_price|license|none) and `decay_test`; if mechanism is none, `downstream_relevance` must be `risk_candidate`. Round (3) must include one sourced sentence on whether you would trust retained earnings (buybacks/SBC/M&A) with `downstream_relevance` valuation_input or risk_candidate — not context_only. Do **not** drop bear-case coverage on deep names.
...
Research the assigned topic using web search. Return JSON only (do not write files). Do not open other research sessions or write `S/data/raw_sec` / transcripts (Agent 2b / 2e). Decision-grade contract: 3-6 specific factual findings beat generic filler; every quantitative claim needs a source; full articles stay on the web/cache — do not paste multi-page text into the return.
...
- S/registry/street_estimates.json — vendor FY+1 and FY+2 **consensus revenue and EPS** (yfinance revenue_estimate/earnings_estimate and/or sp_data estimates). Label company_fy vs calendar. This is Agent 5's **default Y1 start** (`street_baseline`), not company guidance and not a price-target path. Schema: ROOT/harness/schemas/street_estimates.schema.json. If the fetch fails, set unavailable=true and log the failure — do not invent consensus. Fetch EBIT/OI when the vendor has it. Record `n_revenue` / `eps_basis`.
...
- _unlabeled/: line-range read at most 5 txts; label via ingest --label. If a labeled doc fills a required-set slot, copy the txt into S. Ignore-with-log only for kind=other / duplicate hash. Unlabeled that still matches session_missing at finish → FAIL. Do not walk filings/ to mine footnotes.

REQUIRED fetch set (if not already bound):
1. At least the **three most recent annual reports** (US: 10-K; non-US: 20-F / AR / annual equivalents). When `research_brief.research_depth` is `deep` **or** `market_context.intensity` is `high`, fetch **five** annuals when they exist. Extra years are not a substitute for ownership/control depth in 1c.
...
Store a cleaned `.txt` sidecar next to HTML/PDF (doc_text.py) so year-readers can section-walk without pasting HTML. Never copy a PDF/HTML-only file into raw_sec.

If session_missing is non-empty: Primary = kimi-datasource plugin (get_data_source_desc("sec_edgar")). Fallback: local sec-edgar MCP (search_company_by_ticker, get_latest_filings, then get_filing_text — it returns only a 20k-char preview in the MCP response; read the COMPLETE filing from the `full_text_path` it returns). Note any substitution. Do not use MCP to re-pull bound years.

HERMETIC STORE (required — **session_missing[] only**):
- Copy newly fetched complete text into S/data/raw_sec/ with stable filenames (accession or form+period). Deep mining and audit re-reads MUST work from the session tree, not only a global MCP cache. Bound files are already the working copy.
- Write-through those new downloads via ingest_library.py --from-file (session path as --from-file).
...
Log S/registry/data_fetch_log.json including freshness: checked_at, index_source, index_path, session_missing, fetched_new. Bind-time fields do not count. Gaps so 2e can mark strategy_arc / scorecard coverage degraded.
```
...
Inputs: S/registry/sec_filings.json, S/data/latest_supplement.* (the uncapped supplement from 2b — use it first), S/data/sp_financials.csv, S/registry/sector_config.json, S/registry/data_fetch_log.json (its downstream_instructions tell you which gaps to fill), S/registry/street_estimates.json (beat/miss vs Street — do not web-hunt consensus), S/registry/news_sentiment.json (do not re-search the earnings headline if EX-99 / supplement exists). Fetch the earnings-release/press summary via kimi-datasource sec_edgar or web search if not already covered.
...
## Phase 1d — operating-path evidence (after 0+1b; before Agent 5)
...
Read: S/registry/background.json, S/registry/latest_quarter.json, S/registry/research_brief.json, sector module. Require, with sources or explicit missing→widen: industry capex vs history, utilization or book-to-bill analog, named competitor supply response, destock vs end-demand, dated **units** (not consultant dollar TAM). Do not write a TAM/SAM/SOM paragraph. Network only to fill `named_gaps[]`. Persist `background_round_ids` or `named_gaps`. Node ramp vs destock is one example, not the whole lens.
...
Read all S/registry/raw/oppath_*.json. Persist they already exist. If S/registry/street_estimates.json exists, map Street vs company run-rate/guide into conflicts[] or hints — still **non-binding numbers**; do **not** average Street with destock into one CAGR. Build per ROOT/harness/schemas/operating_path_brief.schema.json: sources.workers (3 paths), conflicts[] (flatten vs destock stays unresolved if both evidenced), `analog_class` (`inventory_channel` | `advertiser_budget` | `none`), `current_print_is_destock` (this print), scenario_hints (if destock and duration are both evidenced: Street FY+1 is a **prior**, destock analog in **bear** unless `current_print_is_destock` — Agent 5 decides per §10c; destock-in-base is legal only when analog_class is set **and** current_print_is_destock=true; capex-crushed IR FCF is not destock-math), rejected_shapes[], recommended_for_agent5 (qualitative; any illustrative path labeled non-binding), verify_rechecks[] ≥3 against CSV/LQ/raw.

Handoff S/registry/handoffs/1d_merge.md.
```

Orchestrator: `preflight --phase 1d` before workers; `preflight --phase 1d --mode complete` before flipping 1d complete / spawning 1e.

---

## Phase 1e — story lock (after 0+1b+1c+1d; before Agent 5)

Harness ≥ 3.4.0. Spawn `story` first, then `3p`. Do not spawn Agent 5 until `preflight --phase 1e --mode complete`. Agent 5 does not write `narrative_bind.json`.

### Agent story — narrative writer (`coder`)

```text
Write the investor's story for TICKER (session S). Anti-role: do not compute a DCF; do not write valuation_model.json; do not change classification.json.

Read: S/registry/classification.json (REQUIRED — identity; do not copy playbook/stage/buyer/contest onto this bind; do not reclassify), S/registry/research_brief.json, S/registry/background.json, S/registry/latest_quarter.json, S/registry/filing_deep_dive.json, S/registry/operating_path_brief.json, S/registry/sector_config.json (KPI only), S/registry/market_context.json. Open ROOT/ref/Investment Valuation/knowledge-hub-narrative/00-router.md then process/build-story.md, 3p-test.md, map-inputs.md. Do not start in extracts/.

Write S/registry/narrative_bind.json per ROOT/harness/schemas/narrative_bind.schema.json:
- story.paragraph (>=20 chars, novice-restatable, beginning-middle-end)
- map keys MUST be story carriers: tam, share_path, slice, target_om, tax_path, sales_to_capital, kc_path, p_fail, tv_form, stable_roc_vs_kc (at least 3). Values are story sentences, not model field names.
- Do not write 3P buckets (the critic owns narrative_3p.json). Do not copy classification stamps.
- evidence_hooks: one row per required finding id (`fdd.<material_key>`, `fdd.strategy_arc`, `oppath.brief`, `oppath.conflict.<id>`). action=map (with map_key) or action=reject. `from` must equal the id. noted_only is illegal.
- stage_fit (>=40 chars): why THIS paragraph matches classification.life_cycle_stage
- street_restory when street_estimates.json exists (will_own true/false)
- When classification.iv_playbook is contingent_claim or overlays include real_options: write option_screens (exclusivity/materiality/no_double_count all true + rationale). When overlays include distressed_call: write distressed_call (dcf_equity_near_zero and replaces_dcf_equity true). These overlay screens live on the bind, not on classification.json.

Handoff S/registry/handoffs/story.md.
```

### Agent 3p — 3P critic (`coder`)

```text
Grade the story lock for TICKER. You are the evaluator, not the author. Anti-role: do not rewrite story.paragraph; do not compute FV; do not reclassify the engine.

Read S/registry/narrative_bind.json (must exist), S/registry/classification.json, the same gather files the story agent used. Open ROOT/ref/Investment Valuation/knowledge-hub-narrative/process/3p-test.md, checks/fairy-tale.md, iron-triangle.md, life-cycle-fit.md.

Write S/registry/narrative_3p.json only per ROOT/harness/schemas/narrative_3p.schema.json: verdict PASS|FAIL; non-empty possible/plausible/probable; fairy_tale; wrong_stage; iron_triangle pass|fail; rationale; required_edits on FAIL. Do **not** write narrative_bind.json. Lock is verdict=PASS.

On FAIL: list required_edits. Orchestrator may respawn story (max 2 loops) then stop as too_hard.

Handoff S/registry/handoffs/3p.md.
```

Orchestrator: `preflight --phase 1e` before story; `preflight --phase 1e --subagent 3p` before the critic; `preflight --phase 1e --mode complete` before Agent 5.
...
**Anti-anchoring:** Derive FV/MoS/probs only from **this session** (`S/data`, `S/registry`, compute scripts). Bound filings are `S/data/raw_sec/*.txt` — do not follow `library_bind` `library_text` paths or re-download annuals. Do **not** open or target other `archive/research/<TICKER>/<other_session_key>/` valuation models, snapshots, or report verdicts to “stay consistent” with a prior run. Same-session upstream is 2a–2e + 1d + 1e (`classification.json` + `narrative_bind.json` + `narrative_3p.json` PASS) + street_estimates (when present). Agent 4 is isolated; Agent 12 is **parallel** — do not wait for TSR. If `S/registry/tsr_validation.json` already exists, read it and you may write `tsr_hooks[]`; do not block the DCF on Agent 12. Compute TTC ROIC from `S/data/sp_financials.csv` + FDD yourself. Prior-day research is not an input.
...
Load current policy: ROOT/harness/RESEARCH_AGENTS.md §10c (Street/Y1 + destock), §10d (ROIC), §10e (WACC). Do not restate those sections here. Do not load ROOT/harness/law_history.md (Agent 13 only, version-banded). On harness ≥ 3.4.0: **compute only.** Read S/registry/classification.json (identity) and S/registry/narrative_bind.json (story + map) and S/registry/narrative_3p.json (must be PASS). Do **not** write narrative_bind.json. Do **not** reclassify. Open the ref recipe `valuation_router.md` names for classification.iv_playbook and each overlay. Write valuation_model.story_input_bind mapping every narrative_bind.map carrier to a real model input name. Write valuation_model.buyer_dials from classification.buyer.class (illiquidity_discount=0 for a public diversified buyer; total_beta + discount >0 for private_undiversified; ke_stepdown for vc_pe). Do **not** re-walk FDD or operating-path into filing_deep_dive_hooks / operating_path_hooks — 1e evidence_hooks already consumed them. Sector/region modules are KPI/stress — not the engine.
Judgment style: read ROOT/harness/exemplars/rationale_quality.md, hooks_quality.md, and valuation_decision_quality.md (GOOD patterns; do not copy illustrative numbers). Schema: ROOT/harness/schemas/valuation_model.schema.json.

Inputs: S/registry/classification.json (REQUIRED on ≥ 3.4.0), S/registry/narrative_bind.json (REQUIRED on ≥ 3.4.0), S/registry/narrative_3p.json (PASS), S/registry/sector_config.json, S/registry/market_context.json (REQUIRED for new sessions), S/registry/research_brief.json (when present), S/registry/latest_quarter.json (read cash_quality when present — you do not write it), S/registry/filing_deep_dive.json (REQUIRED), S/registry/operating_path_brief.json (REQUIRED on harness ≥ 2.6.0), S/registry/street_estimates.json (REQUIRED on harness ≥ 2.7.0 unless data_fetch_log records a Street fetch failure), S/registry/data_fetch_log.json, S/data/sp_financials.csv, S/data/peer_comparison.csv, S/registry/background.json, sector module in sector_config.module_file, region module in market_context.module_file (both advisory). Prefer S/data/price_snapshot.json for current price / MoS when present (orchestrator freezes it before Phase 2). Fill peer KPI gaps from S/data/raw_sec, FDD, or sec_filings.json — if still missing, handoff + widen; do not re-fetch annuals. 1d workers are evidence only — YOU write growth/OM paths from the locked map. Do not average flatten-vs-destock.

PREFLIGHT (stop → handoff status=blocked; do not invent):
- registry/classification.json exists; narrative_3p.verdict=PASS.
...
- priced_for_perfection is a surface reverse-eng claim (name dials that justify price vs base/bull) — never from PW vs price×k or price>base alone. Invert the **Street/print Y1 path** (the tape’s implied company), not a destock DCF. PFP=false does not license `initiate`. Write `reverse_engineering.path_inverted` as `street_y1` | `destock` | `other` when present.
- Do not invent FDD/hooks if deep dive missing.

1. READ the locked engine. Use classification.iv_playbook and overlays as given. Do **not** stamp a different playbook. Sector module is KPI/stress only — do **not** take NAV, ARR, TTC×EBIT, or exit multiple as `fair_value.base`. Empty `module_file` → ordinary DCF **only when §5 already chose `standard`** and the router row is `mature_operating`. Do not paste growth-module decay tables. If the locked map already carries a cyclical overlay, honor it; do **not** rewrite `primary_sector` / `module_file` (branded CPG stays standard; F21). Still honor `research_brief.must_cover_risks` / Phase 0 `risk_candidate` stress seeds. Do not treat one peak year or one trough quarter as mid-cycle. Material multi-line businesses → SOTP or multi-method cross-check; if so, write multi_method_reconciliation (primary_fv_for_decision, cross_check_fv, delta_pct, why_primary_wins, what_would_flip_primary). Never average/blend DCF and a multiple. Justify in model.rationale. Write `story_input_bind`.
1b. MARKET CONTEST is already on classification.json — do not rewrite it. Still interrogate a large FV-vs-price gap (model, then inputs) before claiming mispricing; `reverse_engineering` names the dials the tape implies. If classification.iv_playbook is `contingent_claim`, honor `option_screens` on the bind if present. If distressed_call screens already passed, the call **replaces** DCF equity, never adds to it. Write `buyer_dials`.
2. DECIDE every assumption: discount rate build-up (cash-flow currency vs discount-rate currency match or explicit FX policy; beta/Rf series scripted), growth/margin paths, terminal approach, CoC/governance dials (may be 0). Each {value, rationale, basis}. Write `terminal_consistency` (required on ≥ 3.1.0): method is Gordon / liquidation / excess_return (financials), never exit/ARR/NAV. Include wacc_minus_g or ke_minus_g, reinvestment/payout identity, tv_share_of_ev_base; if TV share >0.75 extend years / lower terminal / widen range. Write the truncation assessment (≥ 3.3.0): `truncation` is required — p=0 with `why_not_material`, or p>0 with the weighted base. Write `per_share_bridge` (≥ 3.3.0). On `wacc_buildup` (≥ 3.3.0) set `erp_method` (implied default; `not_applicable` illegal while applies=true; historical needs `erp_reject_implied_reason`), `beta_method` (bottom_up default; `not_applicable` illegal while applies=true; otherwise `beta_reason`), and `discount_currency`/`cash_flow_currency` (match or `fx_policy`). Optional `pricing` object for NAV/ARR/TTC×EBIT/exit multiple — labeled price, not the decision number. Footnotes for SBC/dilution, tax, debt/leases, segments when extracted. NEVER paste region-module ranges as mandated WACC/ERP/family discounts.
2b. Write `roic_identity` per §10d (schema + valuation_decision_quality.md ROIC pair). Agent 12 is not this identity.
2c. Write `wacc_buildup` per §10e. Do not paste a WACC/ERP/Kd number.
...
4. On ≥ 3.4.0 do **not** write filing_deep_dive_hooks[] — 1e evidence_hooks already mapped or rejected each material FDD id. Honor those rejects in the forecast. Credibility may still move weights/range width — not formulas. Degraded scorecard → widen range and say so. Scorecard **beat is not a hit** for `trust_guides_more` unless you also report a met-only and/or cash/organic quality split (harness ≥ 2.11.0 WARN). On < 3.4.0: CONSUME deep dive → filing_deep_dive_hooks[].
...
4d. On ≥ 3.4.0 do **not** write operating_path_hooks[] — 1e evidence_hooks already mapped or rejected `oppath.brief` and each conflict. Destock-in-base vs Street: §10c (do not restate here). On < 3.4.0: operating_path_hooks[] not all noted_only.
4e. Write street_bind + street_hooks when street_estimates.json exists. Policy: §10c. Conservatism_dials and SOTP/DCF multi_method_reconciliation: schema + §10c.
5. Write S/data/compute/valuation.py (session-relative), hermetic from session files, run it; JSON matches script output. Emit dual MoS from the script when possible. On harness ≥ 2.44.0 the script MUST define `fair_value_under(overrides)` and accept `--under '{...}'` printing `{"fair_value": <float>}` without rewriting valuation_result.json. Overrides are **replacement** values for named dials (missing keys = current). `fair_value_under({})` equals fair_value.base. Write `shock_surface.dials` as `{<name>: {value, lo?, hi?}}`. The 4×4 grid is display, not the stress engine. Policy: RESEARCH_AGENTS.md §10f.
...
OUTPUT CONTRACT — write S/data/valuation_model.json per ROOT/harness/schemas/valuation_model.schema.json including: model, fair_value (bear/base/bull/PW + dual MoS), assumptions, compute_script, sensitivity, market_context_hooks (when MC present), street_bind + street_hooks (when street_estimates present), conservatism_dials (required on harness ≥ 2.7.0), reverse_engineering; plus **on ≥ 3.4.0: `story_input_bind` + `buyer_dials` (do not write narrative_bind)** / on < 3.4.0 filing_deep_dive_hooks + operating_path_hooks + narrative bind (`iv_playbook`; story/map/3p) / terminal_consistency (required on ≥ 3.1.0; Gordon/liquidation/excess_return; a present `truncation` block with `p>0` must write `fair_value.base = (1−p)×GC + p×failure` on ≥ 3.2.0) / optional pricing column (≥ 3.2.0: `used_as`/`role` is a closed price-only set — `cross_check | relative_reference | sanity_check | reference_only`) / multi_method_reconciliation (required when SOTP and DCF both run) / research_brief_hooks / decision_usefulness / probability_method when triggered / **roic_identity (required on harness ≥ 2.8.0)** / **wacc_buildup (required on harness ≥ 2.40.0)** / **explicit_forecast (required on harness ≥ 2.43.0)** — copy the script's year-by-year path (revenue / operating profit / margin / capex / FCFF or native analog arrays) for bear/base/bull; `applies:false` + `not_applicable_reason` ≥40 chars + `native_analog` when the model is not a path. Same floats as `data/compute/valuation_result.json` (rounding is display-only). Fair-value weights become risk_bridge scenario_probabilities (bare floats only; consistent). **shock_surface.dials (required on harness ≥ 2.44.0)**. On ≥ 3.3.0 also: `truncation` assessment, `per_share_bridge`, and `wacc_buildup.erp_method`/`beta_method`/`discount_currency`/`cash_flow_currency`; `narrative_bind` `life_cycle_stage`/`who_leads`/`stage_fit`/`buyer`/`market_contest`/`option_screens`/`distressed_call` as applicable.

SELF-CHECK before handoff: (1) abs(pct − 100×frac) < 0.05; (2) ≥3 rationales rehydrate values; (3) terminal_consistency present; method Gordon/liquidation/excess_return; on ≥ 3.4.0 classification.iv_playbook is the engine, story_input_bind maps every carrier, buyer_dials match classification.buyer.class, narrative_3p.verdict=PASS, and you did not write narrative_bind; on < 3.4.0 `iv_playbook` stamped on the bind; any `truncation` block with `p>0` writes `fair_value.base = (1−p)×GC + p×failure` (no `material` flag); (4) PFP names dials or explicit FALSE with dial evidence; (5) ERP/method competitor named if CoE used; (6) street_bind / roic_identity / wacc_buildup match §10c–e and the schema; (7) explicit_forecast Y1 revenue (or native KPI) matches the compute result; (8) shock_surface.dials have current values and `--under {}` equals base FV (§10f); (9) on ≥ 3.3.0 `truncation` is answered, `per_share_bridge` is present, and wacc_buildup has erp_method/beta_method/discount_currency/cash_flow_currency.
Write S/registry/handoffs/5_valuation.md.
```

**5b — decision reopen (orchestrator lead, after Phase 2.5):** Load RESEARCH_AGENTS.md §8 **5b** and §10f. Run `python -m packages.kd_research.stress_apply --ticker T --date D --skip-apply --bind-print`. Write that `stress_bind` object onto `decision.json` only. Change `duration.action` only when `size_cap=0` forbids the verb. Do not rewrite FV or `risk_bridge.json`. Do not spawn subagent `5` in `2_5`. Optional handoff `S/registry/handoffs/5b_decision_reopen.md`.
...
{"name": "...", "type": "sector|macro|region",
 ...
 "shock": {"<dial>": <replacement value, not a delta>},
 "affected_parameters": ["dials in shock"],
 "liquidity_path": {"applies": true|false, "survival_12m": true|false|null, "notes": "..."},
 "overlaps": ["optional other scenario names that are the same movie"],
...
Do **not** guess `fair_value_haircut_pct`. Haircut is derived by `fair_value_under` after merge (`python -m packages.kd_research.stress_apply`). Shock values **replace** current `shock_surface.dials` (not deltas). Unknown legal quantum → do not shock; monitoring only. A legal/contingent **dollar** claim must cite FDD `contingencies_legal` (deep_dive_refs) or set unknown. Do not web-research TAM/moat/management/destock. Do not apply a fixed family-control or country discount from a region module.
```

**Merge protocol (main agent):** Role: risk-bridge assembler — not a co-author of new haircuts. (1) write each verbatim return to `S/registry/raw/stress_{id}.json`; (2) merge **shocks only** into `S/registry/risk_bridge.json` per `harness/schemas/risk_bridge.schema.json`: `risks` (every `latest_quarter.json.risks[]` entry must map to a risk here or be explicitly dropped with a reason; fold material deep-dive legal/contingency findings into risks or explicit drops; Phase 0 `risk_candidate` findings map or explicit drop), `scenario_probabilities` (**ONLY** keys `bear`, `base`, `bull` with numeric values summing to 1.0 — mirrors valuation weights; put any rationale/note/_sum in a sibling such as `scenario_probabilities_rationale`, never inside the probability map), `stress_test.probability_semantics` (standalone-conditional convention), `stress_test.scenarios` (`shock`, `liquidity_path` — no guessed haircut); (3) run `python -m packages.kd_research.stress_apply --ticker T --date D` so Apply walks the merged scenario list and writes one `applied` block per row (plus `reverse_stress` via `fair_value_under`). Raws stay the gather archive. Spot-check `applied.fair_value_haircut_pct` against `1 − applied.stressed_fv/base`. Coverage checkpoint: ≥5 raw stress files + ≥5 merged scenarios. A scenario that would bind (`size_cap < 1` after `compute_stress_bind`) **must not** use `narrative_only`.
...
- valuation_football_field.png (bear/base/bull FV, probability-weighted FV, current price — from S/data/valuation_model.json) — required when duration.action is initiate|add|hold|trim|sell|short; when pass|too_hard, omit the bid poster or draw range/floor vs price instead of a target. If `decision.stress_bind.size_cap < 1` and stressed_fv exists, mark stressed FV as a label (stress, not a bid)
- sensitivity grid heatmap from valuation_model.json sensitivity block; tornado uses derived haircuts from risk_bridge (not worker guesses)
...
Read: all of S/registry/ (sector_config, market_context, research_brief when present, background, sec_filings, filing_deep_dive, news_sentiment, latest_quarter, tsr_validation, risk_bridge, data_fetch_log), S/data/valuation_model.json, S/data/peer_comparison.csv, S/data/raw_sec/*.txt as needed. Skim sec_filings.json sections as needed; use filing_deep_dive.json for footnotes/strategy/scorecard (do not re-hallucinate from web when deep dive has primary excerpts). Do not re-download 10-Ks. Use market_context.json for listing/accounting/ownership/intensity (do not invent regional haircuts not in the valuation model).

Writing: follow ROOT/harness/exemplars/report_prose_quality.md. Short sentences. Everyday words. One idea per sentence. Lead with the product English duration label for `duration.action` (Do not initiate / Hold / … — same map as duration_label). The machine token may appear once as a source line, not the headline. Do not invent a second verb. Do not invent a third duration phrase the gate cannot recognize.

Structure:
- executive summary & verdict (product English duration label first; do not invent a second verdict)
...
Read S/registry/decision.json when present and lead the verdict with the product English duration label. Do not invent a second action. ATR share-count is not a cover position size.

Cover, in this order (harness ≥ 2.43.0 CIO page): (1) lead with the **product English duration label** for `duration.action` (Do not initiate / Hold / Initiate / Add / Trim / Sell / Short / Too hard — same map as duration_label) plus one-sentence why from `decision.rationale`. The machine token may appear once as a source line, not the headline. Do not invent a second verb. Do not invent a third duration phrase the gate cannot recognize.; (2) cheap_claim in product English (Franchise MoS / Equity near book / Residual option / Not cheap); (3) then **Unstressed vs stressed** (harness ≥ 2.44.0; heading exactly `## Unstressed vs stressed`) — copy `format_stress_card`. Do not recompute. When `fair_value.decision_usefulness` is **low**, lead with the **cone + pass**, not the point FV. When `size_cap < 1`, lead with the duration verb + this card, not the point FV. Then fair value vs price + margin of safety as **context** (illustrative going-concern, not a bid, when action is pass/too_hard or cheap_claim ≠ franchise_mos). (4) TA overlay side — not the duration book; (5) sector/market identity headers; latest-quarter headline; key risks (top 3); required inputs AS EXECUTED; data-quality notes; pointer to the other two reports. Do **not** lead with fair value vs price + MoS or a bull/base/bear cover verdict. Leave a one-line placeholder for the audit verdict — Phase 5 will fill it with **Audit PASS = process completeness (provenance/hooks), not an investment recommendation or buy list.** Do not put ATR share-count on this cover as a position size.
```
...
2. EXTERNAL VERIFICATION: ≥5 filing-grade numbers vs issuer/EDGAR/exchange **for the same accession already on disk in S/data/raw_sec** (session copy ≠ truth). Do not verify against other research sessions. Consistency is not truth. ≥1 of the ≥5 must be multi-period series or historical stress anchor (not five lines from the same EX-99.1 only). Also ≥3 re-checks against S/data/raw_sec/*.txt.
2b–2d. Deep-dive structure + re-checks; consumption; market_context hooks + intensity gate (high intensity all-noted_only is FAIL-quality). On ≥ 3.4.0 grade `narrative_bind.evidence_hooks` (each FDD/oppath finding id mapped or rejected). On `< 3.4.0` grade `filing_deep_dive_hooks` / `operating_path_hooks` not all-noted_only. Still grade *substance* of use/reject.
...
2h. **Agent 4 isolation:** technical.json / handoffs/4* must not cite fundamental paths (valuation_model, filing_deep_dive, background, latest_quarter, market_context, sec_filings, sp_financials, street_estimates, library_bind, archive/library). Machine may FAIL under --full; confirm independence of TA lens.
...
4-street. Grade Street/Y1 + destock against RESEARCH_AGENTS.md §10c (Street FY+1 is the default Y1 forecast on ≥ 3.0.1; independent_y1 needs a named gate). If `harness_version < 3.0.1`, stop and load ROOT/harness/law_history.md — do not paste 2.7–3.0.0 Street statutes into this prompt.
4-engine. On harness ≥ 3.1.0: `iv_playbook` matches `valuation_router.md`; `fair_value.base` is DCF (or equity DCF), not NAV/ARR/exit/TTC×multiple; `terminal_consistency.method` is Gordon/liquidation/excess_return. Sector module used as KPI/stress only. If `harness_version < 3.1.0`, skip this item (omit-TV was legal). You do not re-value.
4-roic. Grade `roic_identity` against §10d + valuation_decision_quality.md ROIC pair.
4-wacc. Grade `wacc_buildup` against §10e. REIT on ≥ 3.1.0 must not skip WACC because NAV/AFFO.
4-stress. Grade shock surface / `stress_bind` / Unstressed vs stressed card against §10f and §8 5b. Follow the session `harness_version` (2.29–2.43 = 25/15 cliff; ≥ 2.44 = book bind). Do not FAIL a pre-2.44 session for missing `size_cap`.
4-midcycle. Grade `mid_cycle_construction` against §10d.7.
4-tier1. On harness ≥ 3.3.0: grade life-cycle + `stage_fit` against the asset's actual stage (a wrong-stage story is major); `buyer` against the marginal investor; `market_contest` present and any big FV-vs-price gap survived model→input interrogation before a cheapness claim; `option_screens` and the distressed-call replacement (call replaces DCF equity, never adds); `truncation` answered; `per_share_bridge` and `wacc_buildup.erp_method`/`beta_method`/currency match. On ≥ 3.4.0 those identity fields live on `classification.json` (`stage_fit` and overlay screens on the bind). If `harness_version < 3.3.0`, skip this item.
4-story-lock. On harness ≥ 3.4.0: compute honored `classification.json` (playbook/stage/buyer) and `narrative_3p.verdict=PASS`; map keys are story carriers; `story_input_bind` maps them onto the model; `buyer_dials` match the classified buyer. Agent 5 writing or restorying `narrative_bind` is major. If `harness_version < 3.4.0`, skip this item.
