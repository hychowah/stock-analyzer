...
**Why good:** States build-up, cites scripted intermediate, explains deviations, ties to market_context. A reader can recompute or disagree with a specific step. After-tax Kd here is a **current** claim at a non-trough 12% debt weight — not a historical coupon.

### BAD (coupon-as-Kd / trough weights)

```json
{
  "wacc": {
    "value": 0.06145,
    "rationale": "WACC matches CAPM buildup: Rf 4.78% + Blume beta 0.81 × 5% ERP + 25bp sleeve; Kd 4.0% pretax from coupon buckets; market weights 51/49. No silent pad above buildup.",
    "basis": "valuation.py; FDD debt note coupons 3.3–4.2%"
  }
}
```

**Why bad:** Matching the formula is not conservative when pretax Kd is a **coupon below Rf** and `wd` is the crushed equity tape. That stack inflates PV, inflates Gordon TV, and can license `franchise_mos` against a junk hurdle. Coupon is a cross-check; Kd is current yield or Rf+spread. Distressed `current_market` weights are not a going-concern structure. Do not copy these numbers.
