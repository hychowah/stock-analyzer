...
**HARD RULE — spawn or abandon:** Every specialist (Phase 0 swarm, 2a/2b/2c, 2d, each year-reader, 2e, 1d workers+merge, 4/5/12, 2.5 swarm, 6, 7/8/11, 13) **must** be launched with `spawn_subagent`. You do **not** write their artifacts. Before each spawn:
...
Load current policy: ROOT/harness/RESEARCH_AGENTS.md §10c (Street/Y1 + destock), §10d (ROIC), §10e (WACC). Do not restate those sections here. Do not load ROOT/harness/law_history.md (Agent 13 only, version-banded). On harness ≥ 3.0.0: load ROOT/harness/modules/valuation_router.md then valuation_checks.md. **Lock then compute:** write S/registry/narrative_bind.json (status=locked) BEFORE valuation_model.json. Do not restory while computing. On ≥ 3.1.0 stamp `iv_playbook` from the router (one primary id; on ≥ 3.3.0 `contingent_claim`/`asset_based` are legal), then **open the ref recipe** `valuation_router.md` names for that playbook and each overlay. On ≥ 3.3.0 also stamp `life_cycle_stage` + `who_leads` + `stage_fit`, `buyer`, `market_contest`, and `option_screens` when option-like. On ≥ 3.2.0 the bind must carry substance, not just a status: `story.paragraph` (≥20 chars), a non-empty `map` whose keys name real model inputs (a key under `assumptions` / `wacc_buildup` / `terminal_consistency` / `roic_identity` / `explicit_forecast` / `street_bind`), and non-empty `3p` `possible`/`plausible`/`probable` lists (possible strands stay out of the DCF base). Sector/region modules are KPI/stress — not the engine.
...
1. CLASSIFY on ROOT/harness/modules/valuation_router.md (object → cash flows → life cycle → leverage → job). Stamp `narrative_bind.iv_playbook` to that one primary id; overlays stack (cyclical, truncation, …). Sector module is KPI/stress only — do **not** take NAV, ARR, TTC×EBIT, or exit multiple as `fair_value.base`. Empty `module_file` → ordinary DCF **only when §5 already chose `standard`** and the router row is `mature_operating`. Do not paste growth-module decay tables. If `1d_ind` shows capacity/utilization as the earnings driver, add a TTC/cycle **overlay** and a sector-fit hook — do **not** rewrite `primary_sector` / `module_file` (branded CPG stays standard; F21). Still honor `research_brief.must_cover_risks` / Phase 0 `risk_candidate` stress seeds (commodity-input or protein-supply shocks on a standard name are path/stress work, not a module switch). Do not treat one peak year or one trough quarter as mid-cycle. Material multi-line businesses → SOTP or multi-method cross-check; if so, write multi_method_reconciliation (primary_fv_for_decision, cross_check_fv, delta_pct, why_primary_wins, what_would_flip_primary). Never average/blend DCF and a multiple. Justify in model.rationale.
1b. MARKET CONTEST + HOSTILE REVIEW (≥ 3.3.0): write `market_contest` (accept market rf/ERP/weights; contest company cash flows/margins). Default to the market being right: if base FV is far from price, interrogate model then inputs before claiming mispricing; `reverse_engineering` names the dials the tape implies. If the payoff is contingent/exclusive, use playbook `contingent_claim` + `option_screens` (exclusivity/materiality/no_double_count) — do not probability-weight a DCF for an option. If DCF equity ~ 0 with limited liability, asset-heavy and long-dated debt, `distressed_call` **replaces** DCF equity (option model), never adds to it.
...
4d. When operating_path_brief.json exists: operating_path_hooks[] (used_as with old/new | rejected | noted_only). Material 1d recommendations may not be all noted_only. You may reject the brief; you may not skip consumption. Destock-in-base vs Street: §10c (do not restate here).
