...
7. **MUST spawn specialists** (isolated prompts) for Phase 0 swarm (one spawn per `registry/raw/phase0_*.json`), 2a∥2b∥2c, 2d, each 1c year-reader, 2e, 1d workers+merge, 4∥5∥12, 2.5 swarm (one spawn per `stress_*.json`), 6, 7∥8∥11, 13. Ritual: `record_spawn.py --event launch` → `spawn_subagent` → `--event return` (return without launch FAILs). Set `phase_status.agents[].execution=subagent`. Retry spawn without recording fail if the tool glitched. **`--event fail` (or `abandon_session.py`) only when you cannot launch and would otherwise work inline — then STOP.** Do **not** write that specialist’s artifacts yourself. Inline specialist work is a FAIL (`orchestrator_inline` forbidden; do not retag Agent 5 as `orchestrator_lead` at 5b). Orchestrator-lead work only: listing stamp, bind_library, sector/market/brief, price_snapshot, phase_status, Phase 0/2.5 merge, 5b reopen.
...
15b. **After Phase 2.5 is complete:** run RESEARCH_AGENTS.md §8 **5b** yourself (lead) — `stress_apply --skip-apply --bind-print`, write `stress_bind` on `decision.json` only, change duration only when `size_cap=0` forbids the verb. Then preflight `4_parallel` (reports wait on 2.5). Phase 3 charts wait on `valuation_model.json`, not 2.5 — spawn Agent 6 with `--phase 3` as soon as Agent 5 has written the model. Spawn Agent 4 / 12 with `--phase 2_parallel --subagent 4|12` (they do not wait on 1d). Spawn Agent 5 with `--subagent 5` (waits on 1d + FDD).
