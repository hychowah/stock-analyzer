...
def _apply_y1_policy(
    ...
    """Apply StreetY1Policy bands and response legality after bind identity."""
    ...
    if policy.y1 and policy.keep_independent_illegal and resp == "keep_independent_vs_street":
        out.append(
            (
                "FAIL",
    ...
    if policy.gated:
        if resp == "street_unusable":
            ...
            return out
        if resp == "independent_y1" and policy.independent_y1_ok:
            ...
            return out
        if fail is not None and abs_d > fail:
            out.append(
                (
                    "FAIL",
                    "street_bind.y1_band",
    ...
    if policy.y1:
        if resp != "street_unusable" and fail is not None and abs_d > fail:
            out.append(
                (
                    "FAIL",
                    "street_bind.y1_band",
        ...
        elif resp != "street_unusable" and warn is not None and abs_d > warn:
            out.append(
                (
                    "WARN",
                    "street_bind.y1_band",
...
def check_street_bind(session: Path) -> list[tuple[str, str, str]]:
    ...
    if isinstance(hooks, list) and hooks:
        ...
        if policy.y1:
            independent = (
                policy.independent_y1_ok and resp_early == "independent_y1"
            )
