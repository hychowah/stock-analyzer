...
**Specialist quality:** spawn-or-abandon and 5b → `RESEARCH_AGENTS.md` §8 + `orchestrator_runbook.md`. Street/Y1 → §10c. ROIC → §10d. WACC → §10e. Machine FAILs → `packages.kd_research.check_catalog` (not a second when-table). Agent 4 isolation and Agent 5 single-writer stay process rules here: do not fan out valuers; Agent 4 must not read fundamental artifacts. Historical Street/destock bands: `harness/law_history.md` (Agent 13 when `harness_version < 2.28` only).
