...
4d. When operating_path_brief.json exists: operating_path_hooks[] (used_as with old/new | rejected | noted_only). Material 1d recommendations may not be all noted_only. You may reject the brief; you may not skip consumption. On harness ≥ 2.18.0, when Street FY+1 is usable, destock analog belongs in **bear** (`applies_in=bear_only`); destock-in-base is FAIL. Do not use DU=low / `pass` as a license to destock Y1. `4d` does **not** win `4e`.
4e. STREET FY+1 IS BASE Y1 (harness ≥ 2.18.0). **Start** base Y1 revenue from Street FY+1 vendor mean. Write street_bind (guide × street × base, delta_pct = (base−street)/street, independent_construction.rationale showing Street as the start plus any overlay) and street_hooks[] with action **used_as:fy1_baseline** (used_as:revenue_path / street_mean also legal). **street_bind.street must equal street_estimates FY+1** when numeric (`response=street_unusable` to skip). Machine FAIL if |delta_pct| > 0.05 unless street_unusable. `keep_independent_vs_street` is illegal. Remaining-period company box **low** is a floor for that same period in base; a miss of an unprinted guide is bear. Profit: Street EBIT/OI if fetched; else EPS×shares with a bridge only if eps_basis is gaap|non_gaap; unknown-basis EPS is not EBIT. Haircuts and destock analogs go to **bear/bull/range**, not a new independent Y1. **Required** conservatism_dials[] with four keys (volume_vs_guide, gaap_om_vs_guide, sbc_in_fcff, wacc_vs_buildup); omit is FAIL. Do not silently stack ≥3 of those in base without stacking_justification. If SOTP and DCF both run, write multi_method_reconciliation (omit is FAIL); if |Δ|/primary > 40%, path_reopened + what_changed or gap_rationale.
...
4-street. When street_estimates.json exists (harness ≥ 2.18.0): Y1 must **start from Street FY+1**. Replacing Street with an invented destock/duration stack is **major**. |delta_pct|>5% without street_unusable is major (machine FAIL). `keep_independent_vs_street` is major. Missing used_as:fy1_baseline is major. Destock-in-base while Street is usable is **major**. Destock-in-bear + Street-Y1 is **not** a major. Unknown-basis Street EPS pasted as EBIT/NOPAT is major. Printed remaining-period box low undercut in base while Street sits inside the box is major. (On 2.7–2.17 sessions: copying Street was the major; destock-in-bear + duration-in-base was the major — grade those stamps against that law.) Omit conservatism_dials or omit multi_method_reconciliation when both SOTP and DCF ran is machine FAIL on ≥2.7.0. `trust_guides_more` without a met-only / cash-quality split is a finding (beat is not a hit). two_quarter_rule that raises Y1 into worse FCF/AR/inventory without bear_only is a finding. two_quarter_rule that raises Y1 while destock conflict is live and FCF is ≥0 with inventory/AR down (WC release) is a finding (destock inverse; cannot raise).
