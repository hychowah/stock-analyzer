...
## 10c. Street FY+1 is base Y1 (new runtime ≥ 2.18.0; 2.7–2.17 calibration)
...
Harness **2.7–2.17** independently built FY+1 then calibrated (`used_as:calibration_check`; copying Street was FAIL). Sessions stamped in that range keep that machine. **≥ 2.18.0 inverts Y1 only.**
...
| Harness ≥ 2.11.0: unresolved flatten-vs-destock cannot be duration-in-base unless destock is in base, DU=low, or duration=pass; Street \|delta\|>20% is a calibration WARN (copying Street still FAIL); TV share >60% with Y8 growth ≥8% requires extend/switch; related-party ≥20% cannot stay intensity=low; `research_brief.mode=update` requires facts-only earning_power_changelog (no prior FV/MoS/WACC). Legacy → SKIPPED | `[machine]` `check_session --full` |
