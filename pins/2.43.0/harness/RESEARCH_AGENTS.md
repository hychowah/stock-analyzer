...
3. On re-entry: read `phase_status.json` and `resume_hint` first; set `current_phase` to the first phase that is not `complete`/`skipped`; **do not re-run agents already `complete`** unless fixing an audit FAIL (then reset that agent to `pending` with a note). **5b (after Phase 2.5, orchestrator lead):** edit `registry/decision.json` only. Do not rewrite FV/WACC/MoS. Do not reset `2_parallel` / Agent 5 to pending. Do not spawn subagent `5` in `2_5`. Do not retag Agent 5 `execution` (stays `subagent`). Read `risk_bridge.json` and `tsr_validation.json` if present. Set `reopened_after_stress: true`. If TSR exists, `tsr_seen: true`. Write `stress_bind` `{material, binding_scenario, haircut_pct, probability}`. **Material** = any scenario with `fair_value_haircut_pct ≥ 0.25` (or ≥25 if stored as percent points) **and** `probability ≥ 0.15`. If material: `duration.action` must not be `initiate` or `add`. No override hatch. If TSR `roc_vs_cost_of_capital=fail` and valuation `cheap_claim=franchise_mos` with `quality_bucket=above_wacc`, write `decision.roc_screen_rebuttal` ≥40 chars. Optional handoff `registry/handoffs/5b_decision_reopen.md`. Machine gate: `check_catalog` at `4_parallel` entry / `--full`.
