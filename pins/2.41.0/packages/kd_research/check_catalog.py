...
Path tables live on `phase_graph.PhaseNode` (re-exported from `gates`). This
module says which domain checks run at which when-tag. session_core and
session_full are exclusive supersets for scripts/check_session.py (never run
