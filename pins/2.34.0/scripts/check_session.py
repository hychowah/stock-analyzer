...
def main() -> int:
    ...
    # session_core: always. Absence is SKIPPED, not silent. Agent 4 / isolation
    # severity depends on --full (passed into those helpers).
    for step in (
        ...
        check_filing_deep_dive_hooks_session,
    ):
        step(session)
    ...
    if args.full:
        ...
        def _record_rows(rows: list) -> None:
            for status, check, detail in rows:
                record(status, check, detail)

        for step in (
            check_filing_deep_dive,
            check_year_dives,
            check_operating_path,
            check_street_bind_session,
            check_roic_identity_session,
            check_decision_quality_session,
            check_wave2_decision_session,
            lambda s: _record_rows(check_wave6_reopen(s)),
            check_wave3_epistemology_session,
            check_wave4_destock_session,
            lambda s: _record_rows(check_cash_quality(s)),
            check_risk_bridge,
            check_valuation_mos_units,
            lambda s: check_reports(s, ticker),
            check_audit_verdict,
            check_handoffs,
        ):
            step(session)
