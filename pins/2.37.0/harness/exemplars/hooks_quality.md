...
### BAD — destock-in-base while this print is not destock
...
**Why bad:** Destock-in-base is FAIL unless `destock_this_print` (this JSON does not show that gate). Street FY+1 is the default Y1 start. Historical 2.12–2.17 destock-in-base default: `harness/law_history.md`.
...
**Why good:** Street is the default Y1 (`used_as:fy1_baseline`). Destock analog is `applies_in: bear_only` when this print is not destock. Material OM recommendation is `rejected`. Illustrative numbers are style only. Destock-in-base is legal only with `destock_this_print` (`analog_class` + `current_print_is_destock=true`), not as a free preference.
