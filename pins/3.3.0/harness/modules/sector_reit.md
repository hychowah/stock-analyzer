...
> **KPI / stress library — not the engine.** Playbook is `real_estate` (`valuation_router.md`). Value is after-tax DCF (keep WACC). NAV, FFO/AFFO multiples, and cap-rate marks are the **price** column (`valuation_model.pricing`), never `fair_value.base`.
> **Module Purpose**: NOI, occupancy, AFFO, cap-rate, and stress seeds for cash-flowing property.
...
IF (SIC in [6798, 6792, 5311] OR "REIT" in name OR property assets > 60%):
    IF mortgage securities > 50% of assets:
        ...
        USE MODEL → real_estate / financial overlay; book value + NIM is **price**
    ELSE:
        CLASSIFY → Equity REIT
        IDENTIFY sub-type from property mix / NAICS
        USE MODEL → after-tax DCF (value); NAV + FFO/AFFO is **price**
...
### 2.1 Price column: NAV (Net Asset Value)

> **NAV is price, not intrinsic value.** It is what you would get if all properties were sold at fair value and debt repaid — a liquidation / appraisal mark. Trading price vs. NAV is the most-used **pricing** metric for equity REITs. Write it on `valuation_model.pricing`. `fair_value.base` remains the after-tax DCF (`real_estate` playbook).
...
|--------------------|-----------------|-----------|
| **DCF (FCF-based)** | **After-tax property DCF (value)** + NAV/FFO/AFFO as **price** | Value is the `real_estate` playbook DCF. NAV/FFO multiples go on `valuation_model.pricing`, not `fair_value.base`. |
