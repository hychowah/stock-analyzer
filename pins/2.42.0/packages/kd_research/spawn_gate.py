...
def session_enforces_spawn(session: Path) -> bool:
    ...
    if (session / SPAWNS_REL).is_file() or (session / ABANDON_REL).is_file():
        return True
    return session_since(session, SPAWN_SINCE)
...
def write_abandon(
    ...
    *,
    ...
    Refuses when the session is already frozen history. Discard and spawn-fail
    stay legal on in-progress trees only.
    """
    if session_is_completed(session):
        raise RuntimeError(
            f"refusing to abandon completed session {session} "
...
def check_spawn_session(session: Path) -> list[tuple[str, str, str]]:
    """Spawn ledger / abandon for check_session. SKIPPED on harness < 2.20.0."""
    ...
    if session_is_abandoned(session):
        out.extend(check_abandon(session))
        return out
    if not session_enforces_spawn(session):
        out.append(
            (
        ...
        return out
    out.extend(check_spawn_discipline(session))
    return out
