...
## 10c. Street FY+1 is a prior (default independent Y1)

**Harness ≥ 3.0.0:** Street FY+1 is a restory/calibration **prior**, not the DCF statute. Default `street_bind.response` is `independent_y1` (no gate required). `street_baseline` is legal when you **explicitly copy** Street as the Y1 level — then the 5% identity still applies. Legal gates when you name one: `destock_this_print` | `definition_mismatch` | `native_kpi` | `street_unusable` | `story_fail` | `ttc_midcycle` (cycle/commodity: margin × current revenues). `keep_independent_vs_street` remains illegal. Destock still **never averaged** with Street. Sessions `< 3.0.0` keep Street as default Y1 (`law_history.md` 2.28–2.44). Street **price targets** stay out of the DCF.

This section is **current law** for harness ≥ 3.0.0. Agent 13 loads `harness/law_history.md` when `harness_version < 3.0.0` (Street default Y1 2.28–2.44; required-Street 2.18–2.27; independent-then-calibrate 2.7–2.17). Live Agent 5 / orchestrator must not load that file.
...
4. **`response`:**
   - `independent_y1` — **default** on ≥ 3.0.0. No gate required. `|delta| > 0.20` FAIL without `divergence_rationale` ≥40 chars. Street still must not be all-`noted_only`.
   - `street_baseline` — you copied Street as the Y1 level. `|delta_pct| > 0.05` is FAIL. `|delta| > 0.03` WARN. `street_hooks[]` must include `used_as:fy1_baseline` (or `used_as:revenue_path` / `street_mean`).
...
5. **Legal `independence_gate` values** (optional on ≥ 3.0.0; required on 2.28–2.44 `independent_y1`):
   ...
   - `story_fail` — narrative bind restory rejects the Street-implied story (`will_own=false` or ≥40 char rationale).
   - `ttc_midcycle` — `primary_sector=cyclical` (or commodity overlay): Y1 is cycle-average **margin × current revenues**, not a Street copy.
...
7. **Destock analog:** destock-in-base is **legal** only when `destock_this_print`. Destock-in-base remains **FAIL** while `street_baseline` is claimed and this print is **not** destock. Destock analog in **bear** is always legal. Mid-cycle / TTC may license independent Y1 on ≥ 3.0.0 via `ttc_midcycle` (margin × current revenues).
...
**Current machine gates only.** Version-banded Street/destock/graph history is `harness/law_history.md` (Agent 13 when `harness_version < 3.0.0`). Python still SKIPPED-gates older stamps; live operators do not need those rows. Damodaran identities (`narrative_bind`, bank engine, growth triad, truncation-writes-base): `packages.kd_research.damodaran_gates` on ≥ 3.0.0.
