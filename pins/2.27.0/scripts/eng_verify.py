...
def check_ticker_blocklist() -> list[str]:
    ...
    for name in ("eng", "packages", "apps", "programs", "harness", "archive", "vendor", "pins"):
...
def check_harness_version_bump(root: Path) -> list[str]:
    ...
    current_ver = str(load_harness_identity(root).get("harness_version") or "")

    for rel in changed:
        if not rel.startswith("pins/"):
            continue
        if rel in {"pins/README.md"}:
            continue
        parts = rel.split("/")
        if len(parts) < 2:
            continue
        ver = parts[1]
        if ver == current_ver and version_changed:
            continue
        errs.append(
            f"published pin path changed without a VERSION bump for that pin: {rel}"
        )
    ...
    pin_dir = root / "pins" / current_ver
    pin_version_file = pin_dir / "harness" / "VERSION"
    if not pin_version_file.is_file():
        errs.append(
            f"harness/VERSION is {current_ver} but pins/{current_ver}/ is missing. "
            "Run python scripts/publish_harness_release.py in the same change set."
        )
    else:
        pin_ident = load_harness_identity(pin_dir)
        if pin_ident.get("harness_version") != current_ver:
            errs.append(
                f"pins/{current_ver}/ harness_version "
                f"{pin_ident.get('harness_version')!r} != {current_ver!r}"
            )
        else:
            print(f"  OK pins/{current_ver}/ matches harness/VERSION")
    return errs
