...
duration stays in base). 2.18–2.27 destock-in-bear while Street FY+1 is
usable (destock-in-base FAIL). >= 2.28 destock-in-base is legal when
destock_this_print (analog_class + current_print_is_destock); otherwise
destock-in-base remains FAIL while Street is usable.
"""
...
from packages.kd_research.street_bind import street_y1_usable
...
def session_is_wave3_runtime(session: Path) -> bool:
    return session_since(session, WAVE3_SINCE)


def session_is_wave4_runtime(session: Path) -> bool:
    return session_since(session, WAVE4_SINCE)
...
def check_destock_not_silent_duration(session: Path) -> list[tuple[str, str, str]]:
    ...
    y1_street = policy.y1 and street_y1_usable(session)
    if y1_street:
        if _destock_in_base(vm):
            if policy.gated and destock_this_print(brief):
                return [
                    (
                        "PASS",
                        "destock_base",
                        "harness ≥ 2.28.0: destock-in-base licensed by destock_this_print",
                    )
                ]
            return [
                (
                    ...
                    "usable unless ≥2.28 destock_this_print; destock analog belongs in bear",
                )
            ]
...
def _check_tv_share_218(
    ...
    if not StreetY1Policy.for_session(session).y1:
        return []
...
def check_destock_default(session: Path) -> list[tuple[str, str, str]]:
    ...
    DU=low / pass). 2.18–2.27 while Street usable: destock analog in bear;
    destock-in-base FAIL. >= 2.28 destock-in-base is legal when destock_this_print.
    """
        ...
        if _destock_in_base(vm):
            ...
            return [
                (
                    ...
                    "destock_default",
                    "harness ≥ 2.18.0: destock-in-base is illegal while Street FY+1 is "
                    "usable unless ≥2.28 destock_this_print; put destock analog in bear",
                )
            ]
...
def check_wave3_destock_complete(session: Path) -> list[tuple[str, str, str]]:
    if not session_is_wave3_runtime(session):
        return []
    return check_destock_not_silent_duration(session)


def check_wave4_destock_complete(session: Path) -> list[tuple[str, str, str]]:
    if not session_is_wave4_runtime(session):
        return []
