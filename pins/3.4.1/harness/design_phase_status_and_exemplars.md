...
**Status:** wired. Runtime DAG is `packages/kd_research/phase_graph.py` (`PhaseNode` edges after overlays). `check_session.py` / `preflight_phase.py` read that graph.  
**Date:** 2026-08-04 (runtime home 2026-09-05)  
...
| Readers | Orchestrator on resume; Agent 13 (audit); `scripts/check_session.py` / `scripts/preflight_phase.py` (via `phase_graph`) |
...
| `1e` | Story lock (story then 3p) | **≥ 3.4.0:** `narrative_bind.json` (story/map/hooks) + `narrative_3p.json` PASS. Lock is the critic verdict. Legacy: SKIPPED (Agent 5 still lock-then-compute) |
| `2_parallel` | 4 + 5 + 12 | `technical.json`, `valuation_model.json`, `tsr_validation.json`, 3 handoffs |
| `2_5` | Stress swarm | `risk_bridge.json` (merged shocks → `applied` via `stress_apply`), ≥5 `raw/stress_*.json` (gather archive), `stress_test.reverse_stress`; preflight `--mode complete` |
...
orch → 1b                  # file gate: financials + filings (not 2c)
orch → 1c                  # file gate: filings (not 2c)
0 + 1b → 1d                 # 1d waits on Phase 0 + latest quarter (not 1c)
0 + 1b + 1c + 1d → 1e      # story lock before Agent 5 (≥ 3.4.0)
orch → 2_parallel          # Agent 4/12 ready-sets; Agent 5 waits on 1e
1d → 2_5                   # file gate: valuation_model
1d → 3                     # charts after Agent 5, not after 2.5
2_5 → 4_parallel           # reports after 2.5
4_parallel → 5             # audit after reports, not charts
5 → done
```

Charts (phase `3`) start after Agent 5’s `valuation_model.json` (file gate). Reports start **after 2_5**. Audit waits on reports, not charts.
...
| 7 fundamental / 11 README | `report_prose_quality.md` |
