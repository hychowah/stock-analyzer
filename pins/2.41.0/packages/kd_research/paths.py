...
# Frozen at import from this file's location so tests that patch PROJECT_ROOT
# cannot treat a tmp tree as the live archive (and unlink local-home sqlite).
_CHECKOUT_ARCHIVE = (Path(__file__).resolve().parent.parent.parent / "archive").resolve()
...
def os_local_home() -> Path:
    """Machine-local data (not OneDrive). Override with STOCK_RESEARCH_LOCAL."""
    raw = os.environ.get("STOCK_RESEARCH_LOCAL")
    if raw and str(raw).strip():
        return Path(raw).expanduser().resolve()
    if sys.platform == "win32":
        base = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
        return Path(base) / "StockResearch"
    return Path.home() / ".local" / "share" / "stock-research"


def is_production_archive(output_dir: Path | str | None = None) -> bool:
    """True when this archive is the checkout's live archive/ (not a test tree)."""
    return archive_root(output_dir).resolve() == _CHECKOUT_ARCHIVE


def local_home(output_dir: Path | str | None = None) -> Path:
    """SQLite + start lock. Not research sessions or job packets.

    STOCK_RESEARCH_LOCAL always wins. Test/fixture ARCHIVE_ROOT trees use
    <archive>/.local so tmp catalogs keep working. The live archive uses
    %LOCALAPPDATA%/StockResearch or ~/.local/share/stock-research.
    """
    raw = os.environ.get("STOCK_RESEARCH_LOCAL")
    if raw and str(raw).strip():
        return Path(raw).expanduser().resolve()
    if not is_production_archive(output_dir):
        return archive_root(output_dir) / ".local"
    return os_local_home()


def catalog_sqlite_path(
    output_dir: Path | str | None = None,
    *,
    filename: str = "research_compare.sqlite",
    create: bool = False,
) -> Path:
    """Catalog sqlite file. Production lives off the synced tree; tests stay in archive/catalog."""
    raw = os.environ.get("STOCK_RESEARCH_LOCAL")
    if raw and str(raw).strip():
        preferred = Path(raw).expanduser().resolve() / "catalog" / filename
    elif is_production_archive(output_dir):
        preferred = os_local_home() / "catalog" / filename
    else:
        preferred = catalog_root(output_dir) / filename
    if create:
        preferred.parent.mkdir(parents=True, exist_ok=True)
        return preferred
    if preferred.is_file():
        return preferred
    legacy = catalog_root(output_dir) / filename
    if legacy.is_file():
        return legacy
    return preferred


def analysis_web_local_dir() -> Path:
    """IB book + alt-history sqlite. Prefer local home; keep apps/.local if that book exists."""
    raw = os.environ.get("STOCK_RESEARCH_LOCAL")
    if raw and str(raw).strip():
        path = Path(raw).expanduser().resolve() / "analysis_web"
        path.mkdir(parents=True, exist_ok=True)
        return path
    legacy = PROJECT_ROOT / "apps" / "analysis_web" / ".local"
    if not is_production_archive():
        return legacy
    preferred = os_local_home() / "analysis_web"
    if (preferred / "portfolio.sqlite").is_file() or (
        preferred / "alt_histories.sqlite"
    ).is_file():
        return preferred
    if (legacy / "portfolio.sqlite").is_file() or legacy.is_dir():
        return legacy
