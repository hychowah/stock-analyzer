...
class WorkflowSpecTests(unittest.TestCase):
    ...
    def test_spec_shape(self) -> None:
        ...
        self.assertEqual(p2["label"], "Valuation")
        self.assertEqual(p2["stage"], "decide")
        orch = next(p for p in spec["phases"] if p["id"] == "orch")
        orch_agent = orch["agents"][0]
        self.assertEqual(orch_agent["label"], "Orchestrator")
        self.assertIn(BIND_REL, orch_agent["writes"])
        self.assertIn("data/price_snapshot.json", orch_agent["writes"])
        a2a = next(
            a
        ...
        self.assertIn("registry/street_estimates.json", a2a["writes"])
        self.assertIn("data/sp_financials.csv", a2a["writes"])
        self.assertIn("Conventions for all agents", spec.get("conventions") or "")
        self.assertNotIn("orchestrator", SPECIALIST_ARTIFACTS)
